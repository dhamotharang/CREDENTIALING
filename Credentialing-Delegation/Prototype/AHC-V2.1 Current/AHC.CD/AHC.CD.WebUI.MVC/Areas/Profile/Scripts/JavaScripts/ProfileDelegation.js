...
delegationApp.controller("delegationController", ['$scope', '$timeout', '$http', 'messageAlertEngine', '$filter', function ($scope, $timeout, $http, messageAlertEngine, $filter) {
    ...
    $http.get("/Profile/ProfileDelegation/GetAllTeamLeadsAsync").then(function (value) {
    ...
    $scope.addUser = function (profileId, profileUserId) {
        ...
        if (validationStatus) {
            $http.post('/Profile/ProfileDelegation/AssignProfile?profileId='+ profileId + '&profileUserId='+ profileUserId).
