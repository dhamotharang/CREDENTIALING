    ...
    internal class MasterDataManager : IMasterDataManager
    {
        ...
        public async Task<IEnumerable<Entities.MasterData.Account.Staff.Employee>> GetAllBusinessContactPersonAsync()
        {
            try
            {
               // var data = await repositoryManager.GetAsync<Employee>(s => s.Status != Entities.MasterData.Enums.StatusType.Inactive.ToString() && s.Departments.Any(d => d.Department.Name.Equals("Operation")));
                var data = await repositoryManager.GetAsync<Employee>(s => s.Status != Entities.MasterData.Enums.StatusType.Inactive.ToString() && s.Departments.Any(d => d.Department.Name.Equals("Operation")) && s.EmailAddress != null);
                var sortData = data.ToList();
        ...
        public async Task<IEnumerable<Entities.MasterData.Account.Staff.Employee>> GetAllBillingContactAsync()
        {
            try
            {
                ...
                var data = await repositoryManager.GetAsync<Employee>(s => s.Status != Entities.MasterData.Enums.StatusType.Inactive.ToString() && s.Departments.Any(d => d.Department.Name.Equals("Billing")) && s.EmailAddress != null);
                var sortData = data.ToList();
        ...
        public async Task<IEnumerable<PracticePaymentAndRemittance>> GetAllPaymentAndRemittance()
        {
            try
            {
                ...
                var data = await repositoryManager.GetAsync<PracticePaymentAndRemittance>(s => s.PaymentAndRemittancePerson.Departments.Any(d => d.Department.Name.Equals("Payment and Remmittance")) && s.PaymentAndRemittancePerson.Status != Entities.MasterData.Enums.StatusType.Inactive.ToString() && s.PaymentAndRemittancePerson.EmailAddress != null, "PaymentAndRemittancePerson");
