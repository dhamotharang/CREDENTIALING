function loadTableDataForRejections(GetThisData) {
    $.ajax({
        type: 'GET',
        ...
        success: function (result) {
            ...
            var tdTagEnd = "</td>"
            for (var i = 0; i <= 30 ; i++) {
                $('#RejectionsTableDR').append(trTag + tdTag + data[i].Type + tdTagEnd
                    + tdTag + data[i].Code + tdTagEnd
                    + tdTag + data[i].Code_Description + tdTagEnd
                    + tdTag + data[i].Count + tdTagEnd
                    + tdTag + data[i].Percentage + tdTagEnd
                    + tdTag + data[i].RevenueLoss + tdTagEnd
                    + tdTag + data[i].ProviderCount + tdTagEnd).fadeIn(300);
            }
...
loadTableDataForRejections("RDR1");
$('#showPrevious').click(function () {
    $('#RejectionsTableDR').html('').fadeOut(300);
    var data = loadTableDataForRejections("RDR1");
...
$('#showNext').click(function () {
    $('#RejectionsTableDR').html('').fadeOut(300);
    var data = loadTableDataForRejections("RDR2");
})
