function loadTableDataForProcedureClaimed(GetThisData) {
    $.ajax({
        ...
        success: function (result) {
            ...
            var tdTagEnd = "</td>"
            for (var i = 0; i <= 30 ; i++) {
                $('#MostCommonDiseasesDRTable').append(trTag + tdTag + data[i].DiseaseCode + tdTagEnd
                    + tdTag + data[i].DiseaseDescription + tdTagEnd
                    + tdTag + data[i].DiseaseCount + tdTagEnd
                    + tdTag + data[i].DiseasePercentage + tdTagEnd
                    + tdTag + data[i].ProviderCount + tdTagEnd
                    + tdTag + data[i].PatientCount + tdTagEnd).fadeIn(300);
            }
...
loadTableDataForProcedureClaimed("MCD1");
$('#showPrevious').click(function () {
    $('#MostCommonDiseasesDRTable').html('').fadeOut(300);
    var data = loadTableDataForProcedureClaimed("MCD1");
...
$('#showNext').click(function () {
    $('#MostCommonDiseasesDRTable').html('').fadeOut(300);
    var data = loadTableDataForProcedureClaimed("MCD2");
})
