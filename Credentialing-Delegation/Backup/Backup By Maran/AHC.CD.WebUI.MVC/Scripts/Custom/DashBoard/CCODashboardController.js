...
dashboardApp.controller("CCODashboardController", ["$scope", "$rootScope", "$http", "$filter", "CCOLicenseService", function ($scope, $rootScope, $http, $filter, CCOLicenseService) {
    ...
    $scope.FinalLicense = CCOLicenseService.LicensesList();
    $scope.printAllSectionsData = function () {
        var mywindow = window.open('', $('#hiddenPrintDiv').html(), 'height=800,width=800');
        for (var i in $scope.FinalLicense) {
            if ($scope.FinalLicense[i].Licenses.length != 0) {
                $scope.getLicensTypeData($scope.FinalLicense[i]);
                $scope.$apply();
                var divToPrint = document.getElementById($scope.FinalLicense[i].LicenseType);
                $(".skip").hide();
                $('#hiddenPrintDiv').empty();
                if (divToPrint != null) {
                    $('#hiddenPrintDiv').append(divToPrint.innerHTML);
                    ...
                    // Creating a window for printing
                    //var mywindow = window.open('', $('#hiddenPrintDiv').html(), 'height=800,width=800');
                    mywindow.document.write('<center><b style="font-size:large">' + $scope.FinalLicense[i].LicenseType + '</b></center></br>');
                    mywindow.document.write('<html><head><title>' + $scope.FinalLicense[i].LicenseType + '</title>');
                    mywindow.document.write('<link rel="stylesheet" href="/Content/SharedCss/bootstrap.min.css" type="text/css" />');
                    mywindow.document.write('<link rel="stylesheet" href="/Content/SharedCss/app.css" type="text/css" />');
                    mywindow.document.write('<style>@page{size: auto;margin-bottom:5mm;margin-top:7mm;}th{text-align: center;}</style>');
                    mywindow.document.write('</head><body style="background-color:white"><table class="table table-bordered"></td>');
    ...
    var jsonobject = [{ Name: StateLicensecolumns },
                      ...
                      { Name: CDSColumns },
                      { Name: SpecialityBoardColumns },
                      { Name: HospitalPreviligesColumns },
                      { Name: ProfessionalLiabilityColumns },
                      { Name: WorkersCompColumns },
                      { Name: MedicareInformationColumns },
                      { Name: MedicaidInformationColumns },
                      { Name: CAQHColumns },
                      { Name: UpcomingRecredentialColumns }]
    ...
    $scope.GeneratePDF = function () {
        ...
        for (var i in $scope.Licenses) {
            ...
            if ($scope.Licenses[i].License.StateLicenseStatus != null)
                $scope.Licenses[i].Status = $scope.Licenses[i].License.StateLicenseStatus.Status;
            $scope.Licenses[i].ExpiryDate = (($scope.Licenses[i].License.ExpiryDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.ExpiryDate).getDate() + '/' + ($scope.Licenses[i].License.ExpiryDate).getFullYear();
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.text("State License", 350, 50);
            $scope.headersScript(jsonobject[0].Name, PDFData, doc)
        }
        //End of State License PDF Printing Section       
        //Start of FEDERAL DEA PDF Printing Section
        k++;
        $scope.getLicensTypeData($scope.FinalLicense[k]);
        for (var i in $scope.Licenses) {
            $scope.Licenses[i].LicenseNumber = $scope.Licenses[i].License.DEANumber;
            $scope.Licenses[i].IssueState = $scope.Licenses[i].License.StateOfReg;
            $scope.Licenses[i].ExpiryDate = (($scope.Licenses[i].License.ExpiryDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.ExpiryDate).getDate() + '/' + ($scope.Licenses[i].License.ExpiryDate).getFullYear();
            $scope.Licenses[i].dayLeft = $scope.Licenses[i].License.dayLeft;
        }
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("Federal DEA", 350, 50);
            $scope.headersScript(jsonobject[1].Name, PDFData, doc)
        }
        //End of FEDERAL DEA PDF Printing Section

        //Start of CDS Information PDF Implementation Section
        k++;
        $scope.getLicensTypeData($scope.FinalLicense[k]);
        for (var i in $scope.Licenses) {
            $scope.Licenses[i].LicenseNumber = $scope.Licenses[i].License.CertNumber;
            $scope.Licenses[i].IssueState = $scope.Licenses[i].License.IssueState;
            $scope.Licenses[i].ExpiryDate = (($scope.Licenses[i].License.ExpiryDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.ExpiryDate).getDate() + '/' + ($scope.Licenses[i].License.ExpiryDate).getFullYear();
            $scope.Licenses[i].dayLeft = $scope.Licenses[i].License.dayLeft;
        }
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("CDS Information", 350, 50);
            $scope.headersScript(jsonobject[2].Name, PDFData, doc)

        }
        //End of CDS Information PDF Implementation Section

        //Start of Speciality Board PDF Implementation Section
        k++;
        ...
        // End of Speciality Board PDF Implementaion Section

        //Start of Hospital Previliges PDF Implementation Section
        k++;

        $scope.getLicensTypeData($scope.FinalLicense[k]);
        for (var i in $scope.Licenses) {
            //$scope.Licenses[i].SpecialtyBoardName = $scope.Licenses[i].License.HospitalName;
            $scope.Licenses[i].ExpiryDate = (($scope.Licenses[i].License.AffiliationEndDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.AffiliationEndDate).getDate() + '/' + ($scope.Licenses[i].License.AffiliationEndDate).getFullYear();
            $scope.Licenses[i].dayLeft = $scope.Licenses[i].License.dayLeft;
        }
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("Hospital Privileges", 350, 50);
            $scope.headersScript(jsonobject[4].Name, PDFData, doc)

        }
        // End of Hospital Previliges PDF Implementation Section

        //Start of Professional Liability PDF Implementation Section
        k++;

        $scope.getLicensTypeData($scope.FinalLicense[k]);
        for (var i in $scope.Licenses) {
            $scope.Licenses[i].ExpiryDate = (($scope.Licenses[i].License.ExpirationDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.ExpirationDate).getDate() + '/' + ($scope.Licenses[i].License.ExpirationDate).getFullYear();
        }
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("Professional Liability", 350, 50);
            $scope.headersScript(jsonobject[5].Name, PDFData, doc)

        }
        //End of Professional Liability PDF Implementation Section

        //Start of Workers Compensation Number PDF Implementation Section
        k++;
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("Workers Compensation", 350, 50);
            $scope.headersScript(jsonobject[6].Name, PDFData, doc)

        }
        //End of Workers Compensation Number PDF Implementation Section

        // Start of Medicare Information PDF Implementation Section
        k++;
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("Medicare Information", 350, 50);
            $scope.headersScript(jsonobject[7].Name, PDFData, doc)

        }
        // End of Medicare Information PDF Implemantation 

        // Start of Medicaid Information PDF Implementation Section
        k++;
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("Medicaid Information", 350, 50);
            $scope.headersScript(jsonobject[8].Name, PDFData, doc)

        }
        // End of Medicaid Information PDF Implemantation Section

        // Start of CAQH PDF Implementation Section
        k++;

        $scope.getLicensTypeData($scope.FinalLicense[k]);
        for (var i in $scope.Licenses) {
            $scope.Licenses[i].NextAttestationDate = (($scope.Licenses[i].License.NextAttestationDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.NextAttestationDate).getDate() + '/' + ($scope.Licenses[i].License.NextAttestationDate).getFullYear();
        }
        ...
        if ($scope.Licenses.length != 0) {
            ...
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
            doc.text("CAQH", 350, 50);
            $scope.headersScript(jsonobject[9].Name, PDFData, doc)

        }
        // End of CAQH PDF Implemantation Section

        // Start of Upcoming Recredential PDF Implementation Section
        k++;

        $scope.getLicensTypeData($scope.FinalLicense[k]);
        if ($scope.Licenses.length != 0) {
            for (var i in $scope.Licenses) {
                $scope.Licenses[i].ReCredentialingDate = (($scope.Licenses[i].License.ReCredentialingDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.ReCredentialingDate).getDate() + '/' + ($scope.Licenses[i].License.ReCredentialingDate).getFullYear();
                if ($scope.Licenses[i].License.InitiatedDate != null)
                    $scope.Licenses[i].EffectiveDate = (($scope.Licenses[i].License.InitiatedDate).getMonth() + 1) + '/' + ($scope.Licenses[i].License.InitiatedDate).getDate() + '/' + ($scope.Licenses[i].License.InitiatedDate).getFullYear();
                else
                    $scope.Licenses[i].EffectiveDate = "Not Available";
            }
            if ($scope.Licenses[i].ProviderID == "") {
                $scope.Licenses[i].ProviderID = "Not Available";
            }
            if ($scope.Licenses[i].GroupID == "") {
                $scope.Licenses[i].GroupID = "Not Available";
            }
            doc.addPage();
            var PDFData = angular.copy($scope.Licenses);
        ...
        $scope.getLicensTypeData($scope.FinalLicense[0]);
        // End of Upcoming Recredential PDF Implemantation Section

        doc.save('DashBoard Expiries.pdf');
    };
