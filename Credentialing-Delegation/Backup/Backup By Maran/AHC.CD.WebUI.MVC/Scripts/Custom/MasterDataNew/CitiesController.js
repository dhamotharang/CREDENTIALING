...
masterDataCities.controller('masterDataCitiesController', ['$scope', '$http', '$filter', '$rootScope', 'masterDataService', 'messageAlertEngine', function ($scope, $http, $filter, $rootScope, masterDataService, messageAlertEngine) {
    ...
    $scope.getCities = function (SelectedStateID) {
        ...
        for (var i in $scope.States) {
            ...
            if ($scope.States[i].StateID == SelectedStateID) {
                ...
                for (var i in $scope.AllCities) {
                    if ($scope.AllCities[i].Code == null) {
                        $scope.AllCities[i].Code = "";
                    }
                }
    ...
    $scope.editCities = function (cities) {
        $scope.disableAdd = true;
        $scope.tempCities = cities;
    ...
    $scope.LoadingAjaxforaddingcity = false;
    //------------------- Save City ---------------------
    $scope.saveCity = function (addData, idx) {
        ...
        if (addData.Name && isExist) {
            $scope.LoadingAjaxforaddingcity = true;
                ...
                success(function (data, status, headers, config) {
                    try {
                        ...
                        if (data.status == "true") {
                            ...
                            $scope.LoadingAjaxforaddingcity = false;
                            if (data.cityDetails.Code == null) { data.cityDetails.Code = ""; }
                            ...
                            messageAlertEngine.callAlertMessage("errorCity", "Sorry Unable To Add City !!!!", "danger", true);
                            $scope.LoadingAjaxforaddingcity = false;
    ...
    $scope.updateCity = function (idx) {
        ...
        var updateData = {
            ...
            CountryId: $scope.tempCities.CountryId,
            StateID: $scope.tempCities.StateID,
        ...
        if (isExist) {
            $scope.LoadingAjaxforaddingcity = true;
            $http.post(rootDir + '/MasterDataNew/UpdateCity', updateData).
                success(function (data, status, headers, config) {
                    try {
                        //----------- success message -----------
                        if (data.status == "true") {
                            messageAlertEngine.callAlertMessage("addedCity", "City Details Updated Successfully !!!!", "success", true);
                            $scope.LoadingAjaxforaddingcity = false;
                            if (data.cityDetails.Code == null) { data.cityDetails.Code = ""; }
                            $scope.AllCities[idx] = data.cityDetails;                            
                            $scope.reset();
                            $scope.error = "";
                            $scope.existErr = "";
                        }
