    ...
    internal class PlanManagerEF : IPlanManagerEF
    {
        ...
        public async Task<PlanDataDTO> getPlanDataByID(int PlanId)
        {
            try
            {
                ...
                planDataDTO.PlanData.PlanLOBs.ToList().ForEach(x => { var ActiveSubplans = x.SubPlans.Where(y => y.Status == (StatusType)1).ToList(); x.SubPlans = ActiveSubplans; });
                planDataDTO.PlanContracts = await getAllPlanContractsByID(PlanId);
                planDataDTO = GetProvidersCountWithLOB(planDataDTO);
                return planDataDTO;
            }
            catch (Exception ex)
            {
        ...
        public async Task<object> UpdatePlanData(Plan Plan, DocumentDTO AttachDocument)
        {
            try
            {
                ...
                var planLobRes = planLobRepo.Get(x => x.PlanID == Plan.PlanID).ToList();
                planLobRes.ForEach(y => { var LOBPresent = Plan.PlanLOBs.Where(x => x.LOBID == y.LOBID).FirstOrDefault(); y.StatusType = LOBPresent != null ? StatusType.Active : StatusType.Inactive; y.ReCredentialingDuration = LOBPresent != null ? LOBPresent.ReCredentialingDuration : y.ReCredentialingDuration; planLobRepo.Update(y); });
                await planLobRepo.SaveAsync();
                return new
                {
        ...
        private IEnumerable<PlanLOB> GetPlanLOBDataByPlanID(int PlanID)
        {
            try
            {
                return uow.GetGenericRepository<PlanLOB>().GetAll("LOB").Where(x => x.PlanID == PlanID && x.Status == StatusType.Active.ToString());
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private PlanDataDTO GetProvidersCountWithLOB(PlanDataDTO PlanDataDTO)
        {
            try
            {
                var contractGridRepo = uow.GetGenericRepository<ContractGrid>();
                PlanDataDTO.ProvidersCount = new List<object>();
                PlanDataDTO.PlanData.PlanLOBs.ToList().ForEach(x =>
                {
                    var res = (contractGridRepo.Get(z => z.LOBID == x.LOBID && z.ContractGridStatus != StatusType.Inactive.ToString() && z.Status != StatusType.Inactive.ToString() && z.CredentialingInfo.PlanID == PlanDataDTO.PlanData.PlanID, "CredentialingInfo"));
                    PlanDataDTO.ProvidersCount.Add(new { Providers = res.Select(g => g.CredentialingInfo.ProfileID).Distinct().Count(), LobId = x.LOBID, ContractsCount = res.Count() });
                });
        ...
        public void UpdateSubPlans(List<SubPlan> SubPlans, int? PlanLOBID)
        {
            ...
            var lobrepo = uow.GetGenericRepository<PlanLOB>();
            var subPlansforthisLOB = lobrepo.Find(x => x.PlanLOBID == PlanLOBID, "SubPlans");
            SubPlans.ForEach(x => {
                foreach (var item in subPlansforthisLOB.SubPlans)
                {
                    if (x.SubPlanId != item.SubPlanId) {
                        item.Status = StatusType.Inactive;
                    } 
                }
            });
            lobrepo.Update(subPlansforthisLOB);
            lobrepo.Save();
            subPlanRepo.Save();
            if (TempData.Count > 0)
            {
