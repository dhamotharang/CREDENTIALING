...
namespace AHC.CD.Business.Profiles
{
    internal class PracticeLocationManager : IPracticeLocationManager
    {
        ...
        public async Task<Facility> GetPracticeLocationFacilityByID(int locationID)
        {
            try
            {
                var location = await PracticeLocationRepository.FindAsync(s => s.PracticeLocationDetailID == locationID && s.Status.Equals(StatusType.Active.ToString()), "Facility");
