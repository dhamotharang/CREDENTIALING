...
namespace AHC.CD.Entities.Credentialing.LoadingInformation
{
    public class ContractGrid
    {
        ...
        public ICollection<ContractGridHistory> ContractGridHistory { get; set; }

        public int? DecredentialingReasonId { get; set; }
        [ForeignKey("DecredentialingReasonId")]
        public DecredentialingReason DecredentialingReason { get; set; }

        public string DecredentialingReasonDocPath { get; set; }
        ...
        #region ContractGridStatus

        public string ContractGridStatus { get; private set; }

        [NotMapped]
        public StatusType? ContractGridStatusType
        {
            get
            {
                if (String.IsNullOrEmpty(this.ContractGridStatus))
                    return null;

                if (this.ContractGridStatus.Equals("Not Available"))
                    return null;

                return (StatusType)Enum.Parse(typeof(StatusType), this.ContractGridStatus);
            }
            set
            {
                this.ContractGridStatus = value.ToString();
            }
        }
