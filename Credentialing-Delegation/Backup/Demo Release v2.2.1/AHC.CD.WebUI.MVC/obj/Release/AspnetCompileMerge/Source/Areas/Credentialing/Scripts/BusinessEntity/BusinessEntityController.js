...
BEApp.controller("BEController", ["$scope", '$filter', '$http', "$timeout", function ($scope, $filter, $http, $timeout) {
    ...
    $http.get('/Credentialing/BusinessEntity/GetAllBussinessEntityPlanMappings').
    ...
    $scope.SaveData = function (data) {
        ...
        $http.post('/Credentialing/BusinessEntity/AddBEPlan', { BussinessEntityPlanMapping: temp }).
          success(function (data, status, headers, config) {
              console.log(data);
              if (data.status == 'true') {
    ...
    $scope.Confirmation = function (data1) {
        $http.post('/Credentialing/BusinessEntity/RemoveBEPlan', { BEPlanMappingID: data1.BEPlanMappingID }).
          success(function (data, status, headers, config) {
              console.log(data);
              if (data.status == 'true') {
                  //----------- temp push controller back-end dependency -------------------
