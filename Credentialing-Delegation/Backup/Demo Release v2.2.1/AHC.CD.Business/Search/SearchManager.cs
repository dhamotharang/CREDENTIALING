...
namespace AHC.CD.Business.Search
{
    internal class SearchManager : ISearchManager
    {
        ...
        {
            string includeProperties = "Plan, Profile, Profile.OtherIdentificationNumber, Profile.PersonalDetail,Profile.PersonalDetail.ProviderTitles.ProviderType,Profile.ContractInfoes.ContractGroupInfoes.PracticingGroup.Group,LoadedContracts,CredentialingLogs";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(NPINumber))
                {
                    ...
                    var reCredInfo = credentialingInitiationInfoRepo.GetAll(includeProperties).Where(o => o.Profile.OtherIdentificationNumber.NPINumber != null).ToList();
                    var npi = reCredInfo.Any(a => a.Profile.OtherIdentificationNumber.NPINumber.Equals(NPINumber) && a.Status != AHC.CD.Entities.MasterData.Enums.StatusType.Inactive.ToString());
                    if (!npi)
                        return result;

                    CredentialingInfo providerSearchResult = (from provider in reCredInfo
                                                              where provider.Profile.OtherIdentificationNumber.NPINumber.Equals(NPINumber)
                                                              select provider).First();

                    //if (providerSearchResult.LoadedContracts.First().CredentialingRequestStatus == AHC.CD.Entities.MasterData.Enums.CredentialingRequestStatusType.Approved.ToString())
                    //{
                    //    result.Add(providerSearchResult);
                    //}

                    if (providerSearchResult.CredentialingLogs.Last().Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.DeCredentialing.ToString())
                    {
                    ...
                    if ((providerSearchResult.CredentialingLogs.Last().Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.Credentialing.ToString()) || (providerSearchResult.CredentialingLogs.Last().Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.ReCredentialing.ToString()))
                    {
                        result.Add(providerSearchResult);
                    }
                ...
                else if (!string.IsNullOrEmpty(firstName))
                {
                    ...
                    foreach (CredentialingInfo p in providerSearchResult)
                    {
                        if (p.LoadedContracts.First().CredentialingRequestStatus == AHC.CD.Entities.MasterData.Enums.CredentialingRequestStatusType.Approved.ToString())
                        {
                            result.Add(p);
                        }
                    }
