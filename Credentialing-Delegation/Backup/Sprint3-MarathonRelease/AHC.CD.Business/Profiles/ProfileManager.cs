    ...
    internal class ProfileManager : IProfileManager
    {
        ...
        public async Task UpdateOtherLegalNamesAsync(int profileId, Entities.MasterProfile.Demographics.OtherLegalName otherLegalName, DocumentDTO document)
        {
            try
            {
                ...
                if(document != null && !document.IsRemoved)
                {
                ...
                {
                    //Delete the profile document row
                    profileRepository.RemoveDocument(profileId, otherLegalName.DocumentPath);

                    //save as null path in other legal names
                    otherLegalName.DocumentPath = null;
                    
                    //Delete the existing document
                    if (!String.IsNullOrEmpty(otherLegalName.DocumentPath))
                        documentManager.DeleteFile(otherLegalName.DocumentPath);
                }
        ...
        public async Task<int> AddHospitalPrivilegeDetailAsync(int profileId, HospitalPrivilegeDetail hospitalPrivilegeDetail, DocumentDTO document)
        {
            try
            {
                if (await profileRepository.AnyAsync(p => p.ProfileID == profileId && 
                                                          p.HospitalPrivilegeInformation.HospitalPrivilegeDetails.Any(h => h.AffilicationStartDate.Equals(hospitalPrivilegeDetail.AffilicationStartDate) &&
