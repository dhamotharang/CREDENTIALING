...
profileApp.controller('educationController', ['$scope', '$rootScope', '$http', 'countryDropDownService', 'masterDataService', 'locationService','messageAlertEngine', function ($scope, $rootScope, $http, countryDropDownService, masterDataService, locationService,messageAlertEngine) {
  ...
  */
    $scope.addressAutocomplete = function (location) {
        ...
        $scope.tempObject.SchoolInformation.City = location;                              
        
        if (location.length < 3 && !angular.isObject(location)) {
            $scope.Locations = [];
            $scope.resetAddressModels();
        }
        if (location.length > 2 && !angular.isObject(location)) {                               
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status == "true") {
                        ...
                        if ($scope.visibilitySecondControl == 'addresidencyForView') {                            
                            //$rootScope.visibilitySecondControl = "addedNewResidencyDetailsForView";
                            ...
                            //$rootScope.visibilitySecondControl = "updatedResidencyDetailsForView";
                            data.ResidencyDetails.Specialty = { ID: data.ResidencyDetails.SpecialtyID, Name: SpecialtyName };
                            data.ResidencyDetails.StartDate = ConvertDateFormat(data.ResidencyDetails.StartDate);
                            data.ResidencyDetails.EndDate = ConvertDateFormat(data.ResidencyDetails.EndDate);
                        ...
                        //$scope.ErrorInResidencyDetailsForView = true;                        
                ...
                error: function (e) {
                    //$scope.ResidencyDetailsErrorListForView = "Please Ensure the details and try again";
                    messageAlertEngine.callAlertMessage('ErrorInResidencyDetailsForView' + IndexValue, "", "danger", true);
