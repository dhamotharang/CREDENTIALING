...
namespace AHC.CD.Business
{
    ...
    {        
        ...
        {
            try {
                   var practicingGroups = await repositoryManager.GetAsync<PracticingGroup>(x=>x.PracticingGroupID!=0,"Group");
                   string GroupName = practicingGroups.FirstOrDefault(x => x.PracticingGroupID == contractGroupInfo.PracticingGroupId).Group.Name;

                   if (await profileRepository.AnyAsync(p=>p.ContractInfoes.Any(x => x.ContractGroupInfoes.Any(m => m.PracticingGroup.Group.Name.Equals(GroupName)))))
                {
