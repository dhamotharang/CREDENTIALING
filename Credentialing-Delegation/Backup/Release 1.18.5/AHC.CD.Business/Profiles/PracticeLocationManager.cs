...
namespace AHC.CD.Business.Profiles
{
    internal class PracticeLocationManager : IPracticeLocationManager
    {
        ...
        public async Task AddPracticeProviderAsync(int practiceLocationDetailID, PracticeProvider practiceProvider)
        {
            try
            {
                if(await PracticeLocationRepository.AnyAsync(p => p.PracticeProviders.Any(pr => pr.NPINumber == practiceProvider.NPINumber && pr.Status.Equals(StatusType.Active.ToString()) && pr.Practice.Equals(practiceProvider.Practice))))
