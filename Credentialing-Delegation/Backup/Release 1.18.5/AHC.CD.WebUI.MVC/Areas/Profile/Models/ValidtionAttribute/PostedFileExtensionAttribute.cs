...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Models.ValidtionAttribute
{
    ...
    public class PostedFileExtensionAttribute : ValidationAttribute, IClientValidatable
    {
        ...
        {
            return string.Format(ErrorMessageString, name, AllowedFileExtensions);
        }

        IEnumerable<ModelClientValidationRule> IClientValidatable.GetClientValidationRules(ModelMetadata metadata, ControllerContext context)
        {
            var model = new ModelClientValidationRule { ValidationType = "postedfileextension", ErrorMessage = this.FormatErrorMessage(metadata.DisplayName) };
