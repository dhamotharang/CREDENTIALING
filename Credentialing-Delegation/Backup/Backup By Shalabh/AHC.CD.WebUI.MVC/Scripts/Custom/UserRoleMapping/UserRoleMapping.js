...
//============================Initiation of Angular Module ==========================
var userRoleMappingApp = angular.module("UserRoleMapping", ['ngTable', 'ngTableResizableColumns']);


//============================Service for sucess/error mesages ============================
userRoleMappingApp.service('messageAlertEngine', ['$rootScope', '$timeout', function ($rootScope, $timeout) {
...
userRoleMappingApp.controller("userRoleMappingController", ['$scope', '$timeout', '$http', 'messageAlertEngine', '$filter', function ($scope, $timeout, $http, messageAlertEngine, $filter) {
    ...
    $scope.ifFilter = function () {
        try {
            var bar;
            var obj = $scope.tableParamsSearch.$params.filter;
            for (bar in obj) {
                if (obj[bar] != "") {
                    return false;
                }
    ...
    $scope.getIndexFirst = function () {
        try {
            if ($scope.groupBySelected == 'none') {
                return ($scope.tableParamsSearch.$params.page * $scope.tableParamsSearch.$params.count) - ($scope.tableParamsSearch.$params.count - 1);
            }
        }
        catch (e) { }
    }
    ...
    $scope.getIndexLast = function () {
        try {
            if ($scope.groupBySelected == 'none') {
                return { true: ($scope.data.length), false: ($scope.tableParamsSearch.$params.page * $scope.tableParamsSearch.$params.count) }[(($scope.tableParamsSearch.$params.page * $scope.tableParamsSearch.$params.count) > ($scope.data.length))];
            }
        }
        catch (e) { }
    }
    ...
    $scope.new_search = function () {
        ...
        $http({
            method: "POST",
            url: rootDir + "../Profile/SearchProvider/SearchProviderJson",
            data: {
    ...
    $scope.deactiveProfileInfo = function (profileId) {
    ...
    $scope.reactiveProfileInfo = function (profileId) {
        $scope.SelectedProfileID = profileId;
    ...
    $scope.showProvTypeDiv = function (div_Id) {
        changeProvType();
        $("#" + div_Id).show();
    };
    var changeProvType = function () {
