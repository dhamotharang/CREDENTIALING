...
profileApp.controller('locationcltr', function ($scope, $http, dynamicFormGenerateService, locationService) {
    $scope.demoDate = "/ Date(1422438982354) /";
    $scope.answer = "Yes";
 ...
 */   
    $scope.addressAutocomplete = function (location) {
        if (location.length === 3 && !angular.isObject(location)) {         //As soon as the length of the string reaches 3 and the location is not an object
            ...
            $scope.ZipCode = location.Zipcode + "-_ _ _ _";         //So we check if its an object and if it is then we asign the values like 
            $scope.State = location.State;                          //city name, state name, etc to the respective models
            $scope.Country = location.Country;                      ////which is inturn is displayed to the users
