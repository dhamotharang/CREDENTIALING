profileApp.controller('practiceLocationController', ['$scope', '$rootScope', '$http', '$filter', 'masterDataService', 'locationService', 'messageAlertEngine', function ($scope, $rootScope, $http, $filter, masterDataService, locationService, messageAlertEngine) {
    ...
    $scope.append = function (data) { $scope.tempObject.Name = data;}
    ...
    $scope.validateMidLevelProviderPractitioner = function (dataObject, dataScope) {
        var status = true;
        ...
        else if ($filter('filter')($rootScope.tempObject.FacilityDetail.MidLevels, { NPINumber: dataObject.NPINumber })[0] != null) {
            status = false;
    ...
    $scope.addFacilityMidlevelPractioners = function (dataObject, dataScope) {
        if (!angular.isDefined($rootScope.tempObject.FacilityDetail.MidLevels)) {
            $rootScope.tempObject.FacilityDetail.MidLevels = [];
        }
        if ($scope.validateMidLevelProviderPractitioner(dataObject, dataScope)) {
            $rootScope.tempObject.FacilityDetail.MidLevels.push({
                'PracticeProviderID': $scope.MidLevelProviderPractitioner.PracticeProviderID,
    ...
    $scope.updateFacilityMidlevelPractioners = function (midLevels, tempSecondObject, index, dataScope) {
        if ($scope.validateMidLevelProviderPractitioner(tempSecondObject, dataScope)) {
            midLevels[index] = tempSecondObject;
    ...
    $scope.editShalabhMidLevel = false;
    ...
    $scope.removeMidLevelProviderPractitioner = function (practitioner, visibilityControlPracLoc) {
        ...
        if (visibilityControlPracLoc == "addPracticeLocationNew") {
            $rootScope.tempObject.FacilityDetail.MidLevels.splice($rootScope.tempObject.FacilityDetail.MidLevels.indexOf(practitioner), 1);
        }
        else {
            practitioner.StatusType = "Inactive";
        }
        if ($rootScope.tempObject.FacilityDetail.MidLevels.length < 1) {
            $scope.resetMidLevelProviderPractitioner();
        }
    };
