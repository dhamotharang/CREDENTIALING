...
.run(["$templateCache", function ($templateCache) {
    $templateCache.put('templateTimePicker.html',
            "<input type='text' class='form-control' key-blocker ng-model='inputTime' ng-disabled='disabledInt' time-format show-meridian='showMeridian' ng-focus='open()' />"
...
.directive('keyBlocker', function ($parse) {
    return {
        restrict: 'A',
...
.directive('timepickerPop', function ($document, timepickerState, TimepickerTemplateURL) {
      return {
        ...
        link : function(scope, element, attrs) {
          ...
          //var MeridianValue = false;
          scope.$watch("inputTime", function (value, oldValue) {
              //var addedHour = 0;
              if (!scope.inputTime) {
                  //if (typeof oldValue !== 'undefined' && angular.isDate(oldValue)) {
                  //    if (oldValue.getHours()>12) {
                  //        MeridianValue = true;
                  //    }
                  //}
              element.addClass('has-error');
              } else {
                  //if (MeridianValue && angular.isDate(scope.inputTime)) {
                  //    addedHour = scope.inputTime.getHours() + 12;
                  //    scope.inputTime.setHours(addedHour);
