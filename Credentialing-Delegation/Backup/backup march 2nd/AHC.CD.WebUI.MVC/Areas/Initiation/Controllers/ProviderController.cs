        ...
        public async Task<ActionResult> Index(ProfileViewModel profile)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var authUserId = await CreateAuthUser(profile.ContactDetail.EmailIDs[0].EmailAddress, ConfigurationManager.AppSettings["DefaultPassword"].ToString(), profile.PersonalDetail.FirstName + profile.PersonalDetail.MiddleName + profile.PersonalDetail.LastName);
