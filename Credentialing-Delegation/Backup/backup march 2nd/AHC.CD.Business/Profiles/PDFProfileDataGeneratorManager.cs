...
namespace AHC.CD.Business.Profiles
{
    internal class PDFProfileDataGeneratorManager : IPDFProfileDataGeneratorManager
    {
        ...
        public async Task<Profile> GetProfileList(int profileID)
        {
            try
            {
        ...
        public async Task<string> GetProfileDetailByIdAsync(int profileId)
        {
            try
            {
                var includeProperties = new string[]
                ...
                //convert data into PDFMappingBusinessDataModel
                string pdfMapping=ConvertProfileToPDFMappingDataBusinessModel(profile);
        ...
        private string ConvertProfileToPDFMappingDataBusinessModel(Profile profile)
        {
        ...
        private string ConstructProfilePrimaryData(Profile profile)
        {
            ...
            try
            {
                ...
                if (profile.BirthInformation != null)
                {
                    ...
                    //DateTime birth = DateTime.Parse(profile.BirthInformation.DateOfBirth);
                    profileData.PersonalInfoDateOfBirth = profile.BirthInformation.DateOfBirth.Split(' ')[0];
                ...
                foreach (var item in profile.PracticeLocationDetails)
                {
                    if (item != null && item.IsPrimary == AHC.CD.Entities.MasterData.Enums.YesNoOption.YES.ToString())
                    {
                        ...
                        if (item.OfficeHour != null)
                        {
                            ...
                            if (item.OfficeHour.PracticeDays.Count > 0)
                            {
                                if (item.OfficeHour.PracticeDays.ElementAt(0).DailyHours.Count > 0) {
                                ...
                                if (item.OfficeHour.PracticeDays.ElementAt(1).DailyHours.Count > 0) {
                                ...
                                if (item.OfficeHour.PracticeDays.ElementAt(2).DailyHours.Count > 0) {
                                ...
                                if (item.OfficeHour.PracticeDays.ElementAt(3).DailyHours.Count > 0) {
                                ...
                                if (item.OfficeHour.PracticeDays.ElementAt(4).DailyHours.Count > 0) {
                                ...
                                if (item.OfficeHour.PracticeDays.ElementAt(5).DailyHours.Count > 0) {
                                ...
                                if (item.OfficeHour.PracticeDays.ElementAt(6).DailyHours.Count > 0) {
            ...
            try
            {
                ...
                for (int i = 1; i < largestCount; i++)
                {
                    ...
                    if (profile.StateLicenses.Count > i)
                    {
                        ...
                        if (profile.StateLicenses.Count > i && profile.StateLicenses.ElementAt(i).StateLicenseStatus != null)
                        {
                            suplement.SupplementStateLicenseStatusCode1 = profile.StateLicenses.ElementAt(i).StateLicenseStatus.Title;
                        }

                        if (profile.StateLicenses.Count > i && profile.StateLicenses.ElementAt(i).ProviderType != null)
                        {
