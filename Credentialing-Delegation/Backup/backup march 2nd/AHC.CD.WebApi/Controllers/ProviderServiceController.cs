...
namespace AHC.CD.WebApi.Controllers
{
    public class ProviderServiceController : ApiController
    {
        ...
        {
            List<ProviderDTO> providers = profiles.GetAllUltimateProviders();
            return providers;
       ...
       [HttpGet]
        ...
        [HttpGet]
        public async Task<List<ProviderDetailsDTO>> GetAllProviders()
        {
            return await profiles.GetAllProviderDetails();
        }
        //fetching from flat table for service to UM
        [HttpGet]
        public List<ProviderDTOForUM> GetProviderData()
        {
            return profiles.NewUMService();
        }
