...
namespace AHC.CD.Business.LocationTracker
{
    internal class LocationTrackerManager : ILocationTrackerManager
    {
        ...
        public List<Profile> GetAllProvidersByName(string name)
        {
            ...
            try
            {
                var profiles = profileRepository.Get(p => p.PersonalDetail != null && (p.PersonalDetail.FirstName.Contains(name) || p.PersonalDetail.LastName.Contains(name) || (p.PersonalDetail.MiddleName != null && p.PersonalDetail.MiddleName.Contains(name))) && p.Status != AHC.CD.Entities.MasterData.Enums.StatusType.Inactive.ToString(), includeProperties);
