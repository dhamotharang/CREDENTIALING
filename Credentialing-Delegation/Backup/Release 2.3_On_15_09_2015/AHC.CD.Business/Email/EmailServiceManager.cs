...
namespace AHC.CD.Business.Email
{
    public class EmailServiceManager : IEmailServiceManager
    {
        ...
        public async Task<IEnumerable<EmailInfo>> GetAllActiveFollowUpEmailsAsync()
        {
            ...
            try
            {
                IEnumerable<EmailInfo> activeEmails = await emailServiceRepo.GetAsync(e => e.Status != Entities.MasterData.Enums.StatusType.Inactive.ToString() && e.IsRecurrenceEnabled == Entities.MasterData.Enums.YesNoOption.YES.ToString(), "EmailRecipients.EmailTracker, EmailRecurrenceDetail");
