...
profileApp.controller('professionalAppCtrl', ['$scope', '$rootScope', '$http', 'messageAlertEngine', '$filter', function ($scope, $rootScope, $http, messageAlertEngine, $filter) {
    ...
    $scope.showAffiliationHistory = function (loadingId) {
        if ($scope.affiliationHistoryArray.length == 0) {
            ...
            var historuUrl = "/Profile/ProfileHistory/GetAllProfessionalAffiliationHistory?profileId=" + profileId;
...
//-----------------------End---------------------------------------//
    ...
    $scope.removeProfessionalAffiliation = function (ProfessionalAffiliations) {
        ...
        url = "/Profile/ProfessionalAffiliation/RemoveProfessionalAffiliation?profileId=" + profileId;
        ...
        if (validationStatus) {
            ...
            $.ajax({
                ...
                processData: false,
                success: function (data) {
                    console.log($scope.affiliationHistoryArray.length);
                    if (data.status == "true") {
    ...
    $rootScope.$on('ProfessionalAffiliation', function () {
        if (!$scope.dataLoaded) {
            ...
            $http({
                method: 'GET',
                url: '/Profile/MasterProfile/GetProfessionalAffiliationsProfileDataAsync?profileId=' + profileId
