...
providerApp.controller("providerController", ['$scope','$timeout', '$http', 'locationService', function ($scope, $timeout, $http, locationService) {
    ...
    $scope.BrowserName = DocNotPreviewBrowserName;
    ...
    $scope.setFiles = function (element) {
        $scope.$apply(function (scope) {
            if (element.files[0]) {
                ...
                //console.log($scope.CVFileTempPath);
            } else {
                $scope.selectedCV = {};
            }
        });
    ...
    $scope.setFiles1 = function (element) {
        $scope.$apply(function (scope) {
            if (element.files[0]) {
                ...
                //console.log($scope.ContractFileTempPath);
            } else {
    ...
    $scope.IsPhoneNumberUnique = function (countrycode, number) {
        if (/^\d{10}$/.test(number)) {
            ...
            $.ajax({
                url: "/Validation/IsContactNumberDoesNotExists?Number=" + number + "&CountryCode=%2B" + councode + "&PhoneDetailID=" + 0,
                async: false,
                cache: false,
                contentType: false,
                processData: false,
	    ...
	    $.ajax({
	        ...
	        success: function (data) {
	            ...
	            if (data.providerInitiationStatus) {
	                ...
	                $scope.profileURL = "/Profile/MasterProfile/ProviderProfile/"+data.profileId;
