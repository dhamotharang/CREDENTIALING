...
namespace AHC.CD.Business.Profiles
{
    internal class ProfileUpdateManager : IProfileUpdateManager
    {
        ...
        public List<ProfileUpdatesTracker> GetUpdatesByTrackerId(int trackerId, string status, string modificationType)
        {
            ...
            updates = trackerRepo.GetAll().Where(c => c.Section.Equals(update.Section) && c.SubSection.Equals(update.SubSection) && c.ApprovalStatus == status && c.Modification == modificationType && c.ProfileId==update.ProfileId).ToList();
            return updates;
        }
