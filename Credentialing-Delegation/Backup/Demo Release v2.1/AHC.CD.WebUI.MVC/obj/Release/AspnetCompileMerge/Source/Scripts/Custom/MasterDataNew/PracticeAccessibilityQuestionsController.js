...
masterDataPAQ.controller('masterDataPAQController', ['$scope', '$http', '$filter', '$rootScope', 'masterDataService', 'messageAlertEngine', function ($scope, $http, $filter, $rootScope, masterDataService, messageAlertEngine) {
    ...
    $http.get("/MasterDataNew/GetAllPracticeAccessibilityQuestions").then(function (value) {
        console.log("PracticeAccessibilityQuestions");
        console.log(value);
        ...
        console.log($scope.PracticeAccessibilityQuestions);
    });
    ...
    $scope.savePAQ = function (idx) {
        ...
        for (var i = 0; i < $scope.PracticeAccessibilityQuestions.length; i++) {
            ...
            if (addData.ShortTitle && $scope.PracticeAccessibilityQuestions[i].Title.replace(" ", "").toLowerCase() == addData.Title.replace(" ", "").toLowerCase()) {
        ...
        console.log("Saving PracticeAccessibilityQuestion");
       
        if (addData.ShortTitle && isExist) {
            $http.post('/MasterDataNew/AddPracticeAccessibilityQuestions', addData).
    ...
    $scope.updatePAQ = function (idx) {
        ...
        for (var i = 0; i < $scope.PracticeAccessibilityQuestions.length; i++) {
            ...
            if (updateData.ShortTitle && $scope.PracticeAccessibilityQuestions[i].FacilityAccessibilityQuestionId != updateData.FacilityAccessibilityQuestionId && $scope.PracticeAccessibilityQuestions[i].Title.replace(" ", "").toLowerCase() == updateData.Title.replace(" ", "").toLowerCase()) {
        ...
        console.log("Updating PracticeAccessibilityQuestion");

        if (updateData.ShortTitle && isExist) {
            $http.post('/MasterDataNew/UpdatePracticeAccessibilityQuestions', updateData).
                success(function (data, status, headers, config) {
                    //----------- success message -----------
                    if (data.status == "true") {
