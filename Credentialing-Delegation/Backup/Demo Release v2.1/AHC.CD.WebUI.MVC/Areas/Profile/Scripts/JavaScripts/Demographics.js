...
profileApp.controller('profileDemographicsController', ['$scope', '$rootScope', '$http', '$filter' , 'masterDataService', 'locationService', 'messageAlertEngine',
function ($scope, $rootScope, $http, $filter, masterDataService, locationService, messageAlertEngine) {
    ...
    $rootScope.$on('BirthInformation', function (event, val) {
        if (val) {
            val.DateOfBirth = val.DateOfBirth.split(" ")[0];
        }
    ...
    masterDataService.getMasterData("/Profile/MasterData/GetAllProviderLevels").then(function (val) {
    ...
    $scope.removeOtherLegalName = function (OtherLegalNames) {
        ...
        url = "/Profile/Demographic/RemoveOtherLegalName?profileId=" + profileId;
    ...
    $scope.showOtherLegalNameHistory = function (loadingId) {
        if ($scope.OtherLegalNamesHistory.length == 0) {
            ...
            var url = "/Profile/ProfileHistory/GetAllOtherLegalNamesHistory?profileId=" + profileId;
    ...
    $scope.showHomeAddressHistory = function (loadingId) {
        if ($scope.HomeAddressesHistory.length == 0) {
            $("#" + loadingId).css('display', 'block');
            var url = "/Profile/ProfileHistory/GetAllHomeAddressesHistory?profileId=" + profileId;
            $http.get(url).success(function (data) {
    ...
    $scope.removeHomeAddress = function (HomeAddresses) {
        ...
        url = "/Profile/Demographic/RemoveHomeAddress?profileId=" + profileId;
        ResetFormForValidation($formData);
        validationStatus = $formData.valid();
    ...
    $scope.IsEmailUnique = function (email) {
        ...
        if (/^\S+@\S+\.\S+$/.test(email)) {
            $.ajax({
                url: "/Validation/IsEmailAddressDoesNotExists?EmailAddress=" + email + "&EmailDetailID=" + 0,
                async: false,
                cache: false,
                contentType: false,
                processData: false,
