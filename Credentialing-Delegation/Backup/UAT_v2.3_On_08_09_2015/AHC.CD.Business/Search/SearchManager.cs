        ...
        public List<CredentialingInfo> SearchProviderProfileForDeCred(string NPINumber = null, string firstName = null, string lastName = null, string specialty = null, string IPAGroupName = null, string CAQH = null, string providerType = null)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(NPINumber))
                {
                    ...
                    var providerSearchResultGroupByPlan = from provider in credentialingInitiationInfoRepo.GetAll(includeProperties)
                                                          where provider.Profile.OtherIdentificationNumber.NPINumber.Equals(NPINumber)
                                                          //&& provider.Status != AHC.CD.Entities.MasterData.Enums.StatusType.Inactive.ToString()
                    ...
                    var providerSearchResult = providerSearchResultGroupByPlan.ToList();
                    CredentialingContractInfoFromPlan report = new CredentialingContractInfoFromPlan();

                    foreach (CredentialingInfo p in providerSearchResult)
                    {
                        int flag = 0;
                        foreach (var item in p.CredentialingLogs)
                        {
                            if (item.Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.DeCredentialing.ToString() || item.Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.Dropped.ToString())
                            {
                                flag = 1;
                            }
                        }
                        if (flag == 0)
                        {
                            foreach (CredentialingContractRequest ContractObj in p.CredentialingContractRequests)
                            {
                                List<ContractGrid> GridObjs = new List<ContractGrid>();
                                foreach (ContractGrid GridObj in ContractObj.ContractGrid)
                                {
                                    if (GridObj.Report != null)
                                    {
                                        report = GridObj.Report;
                                    }
                                    if (GridObj.StatusType == AHC.CD.Entities.MasterData.Enums.StatusType.Active && report.CredentialingApprovalStatusType != AHC.CD.Entities.MasterData.Enums.ApprovalStatusType.Rejected && report.TerminationDate > DateTime.Now)
                                    {
                                ...
                                ContractObj.ContractGrid = GridObjs;
                            }
