    ...
    return {
        ...
        link: function (scope, element, attr, ngModel) {
            if (attr.autocomplete == "addressautocomplete") {
                ...
                //    element.parent().find(".ProviderTypeSelectAutoList").first().hide();
                //});
...
ahc_cd_autosearch.directive('countryDialCode', ['$compile', function ($compile) {
    return {
        ...
        link: function (scope, element, attr) {
            ...
            //if (scope.temp && !scope.temp.CountryCode) {
            //    scope.temp.CountryCode = "+1";
            //} else if (!scope.temp) {
            //    scope.temp = {};
            //    scope.temp.CountryCode = "+1";
            //}
        ...
        link: function (scope, element, attr) {
            if (attr.autocomplete == "practitioner") {
                ...
                var contentTr = angular.element(
                    "<div class='ProviderTypeSelectAutoList popover fade bottom in' role='tooltip'>" +
                    "<table class='table table-striped table-bordered table-hover table-condensed marginBottomAutosearch'>" +
                    "<tbody><tr ng-repeat='practitioner in midLevelPractitioners | filter:demodata' ng-click='" + attr.onselect + "(practitioner)' class='pointer'>" +
                    ...
                    "<tr ng-if='midLevelPractitioners.length<1'><td>No Mid-Level Practitioner Available</td></tr></tbody></table></div>");
