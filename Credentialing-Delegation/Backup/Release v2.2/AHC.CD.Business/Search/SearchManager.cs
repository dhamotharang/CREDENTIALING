                ...
                if (!string.IsNullOrEmpty(NPINumber))
                {
                    ...
                    var providerSearchResultGroupByPlan = from provider in credentialingInitiationInfoRepo.GetAll(includeProperties)
                                                ...
                                                group provider by new{
                                                    provider.PlanID,provider.ProfileID
                                                }
                                                    into g
                   ...
                   foreach (CredentialingInfo p in providerSearchResult)
                   {
                       ...
                       {
                           if (item.Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.DeCredentialing.ToString())
                           {
                               flag = 1;
                           }
                       }
                       if (flag == 0)
                       {
                           foreach (CredentialingContractRequest ContractObj in p.CredentialingContractRequests)
                           {
                               ...
                               foreach (ContractGrid GridObj in ContractObj.ContractGrid)
                               {
                                   if (GridObj.StatusType == AHC.CD.Entities.MasterData.Enums.StatusType.Active)
                                   {
