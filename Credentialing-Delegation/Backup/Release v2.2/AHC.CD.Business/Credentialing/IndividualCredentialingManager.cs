...
namespace AHC.CD.Business.Credentialing
{
    internal class IndividualCredentialingManager : IIndividualCredentialingManager
    {
        ...
        public int InitiateReCredentialingAsync(int credentialingInfoID, CredentialingInfo credentialingInitiationInfo, string userAuthId, int[] CredentialingContractRequestsArray, int[] NonCredentialingContractRequestsArray)
        {
            ...
            try
            {
                ...
                var credentialingInitiationConReqList1 = credentialingInitiationConReqRepo.GetAll("ContractSpecialties,ContractPracticeLocations,ContractLOBs,ContractGrid").ToList();
                            ...
                            if (data.CredentialingContractRequestID == CredentialingContractRequestsArray[i])
                            {
                                ...
                                for (int cg = 0; cg < data.ContractGrid.Count; cg++)
                                {
                                    if (data.ContractGrid.ElementAt(cg).Status == Entities.MasterData.Enums.StatusType.Inactive.ToString())
                                    {
                                        ...
                                        {
                                            data.ContractGrid.Remove(data.ContractGrid.ElementAt(cg));
                                        }
                                    }
                                }
                ...
                credentialingLogList.Add(credLog);
                credentialingInitiationInfo.CredentialingLogs = credentialingLogList;
                foreach (var data in credentialingInitiationConReqList)
                {
                    data.ContractRequestStatusType = AHC.CD.Entities.MasterData.Enums.StatusType.Active;
                }
        ...
        public async Task<IEnumerable<Plan>> getAllPlanListforCredentialinginfoByContractRequest(int ProviderID)
        {
            try
            {
                ...
                foreach (var data in credentialingInfo)
                {
                    if (data.ProfileID == ProviderID && data.Plan.IsDelegated == AHC.CD.Entities.MasterData.Enums.IsDelegated.YES)
                    {
                        Plan.Add(data.Plan);
                    }
                }
