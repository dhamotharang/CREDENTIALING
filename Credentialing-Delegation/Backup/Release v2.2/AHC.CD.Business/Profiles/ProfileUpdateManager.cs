    ...
    internal class ProfileUpdateManager : IProfileUpdateManager
    {
        ...
        public List<ProfileUpdatesTracker> GetUpdatesById(int profileId) 
        {
            try
            {
                List<ProfileUpdatesTracker> updates = new List<ProfileUpdatesTracker>();
                var trackerRepo = uow.GetGenericRepository<ProfileUpdatesTracker>();
                var updatedData = trackerRepo.GetAll().Where(c => c.ProfileId == profileId && c.ApprovalStatusType != AHC.CD.Entities.MasterData.Enums.ApprovalStatusType.Approved).ToList();
                return updatedData;
            }
            catch (Exception)
            {
        ...
        public List<BusinessModels.ProfileUpdates.ProfileUpdatedData> GetDataById(int trackerId)
        {
            ...
            {                
                if (item != null && item.NewValue != item.OldValue && (!item.FieldName.Contains("Modified")) && (!item.FieldName.Contains("History")))
                {
                    ...
                    if (!item.FieldName.Contains("VisaInfo") && !item.FieldName.Contains("SocialSecurityNumber") && !item.FieldName.Contains("Option") && !item.FieldName.Contains("Code") && !item.FieldName.Contains("PreferenceType") && !item.FieldName.Contains("Stored"))                       
        ...
        private List<ProfileUpdatedData> ConstructChangesForFederalDEA(dynamic oldDEAData, dynamic newDEAData)
        {
            ...
            if (oldData.ExpiryDate != newData.ExpiryDate)
            {
                ...
                DEA.OldValue = oldData.ExpiryDate.ToString();
                DEA.NewValue = newData.ExpiryDate.ToString();
                updatedList.Add(DEA);
            }
            ...
            if (oldData.IssueDate != newData.IssueDate)
            {
                ...
                DEA.OldValue = oldData.IssueDate.ToString();
                DEA.NewValue = newData.IssueDate.ToString();
                updatedList.Add(DEA);
            }
        ...
        private List<ProfileUpdatedData> ConstructChangesForPracticeLocation(dynamic oldLocationData, dynamic newLocationData)
        {
            ...
            {
                ProfileUpdatedData location = new ProfileUpdatedData();
                location.FieldName = "MobileNumber";
            ...
            var typeRepo = uow.GetGenericRepository<ProviderType>();
            ...
            if (oldData.FacilityDetail.Service.PracticeTypeID != newData.FacilityDetail.Service.PracticeTypeID)
            {
                ...
                {
                    if (item.ProviderTypeID == oldData.FacilityDetail.Service.PracticeTypeID)
                        accessibility.OldValue = item.Title;
                }

                foreach (var item in providerTypes)
                {
                    if (item.ProviderTypeID == newData.FacilityDetail.Service.PracticeTypeID)
            ...
            for (int j = 0; j < 7; j++)
            {
                ...
                if (oldHourCount > 0)
                {
                    oldHourValue = "Start Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime + " - " + "End Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime;

                    for (int i = 1; i < oldHourCount; i++)
                    {
                        oldHourValue = oldHourValue + ", " + "Start Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime + " - " + "End Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime;
                    }
                }
                ...
                if (newHourCount > 0)
                {
                    newHourValue = "Start Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime + " - " + "End Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime;

                    for (int i = 1; i < oldHourCount; i++)
                    {
                        newHourValue = newHourValue + ", " + "Start Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime + " - " + "End Time : " + oldData.FacilityDetail.PracticeOfficeHour.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime;
                    }
                }
        ...
        private List<ProfileUpdatedData> ConstructChangesForOfficeHours(dynamic oldOfficeHoursData, dynamic newOfficeHoursData)
        {
            ...
            for (int j = 0; j < 7; j++)
            {
                int oldHourCount = oldData.PracticeDays.ElementAt(j).DailyHours.Count;
                int newHourCount = newData.PracticeDays.ElementAt(j).DailyHours.Count;
                string oldHourValue = "";
                if (oldHourCount > 0)
                {
                    oldHourValue = "Start Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime + " - " + "End Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime;

                    for (int i = 1; i < oldHourCount; i++)
                    {
                        oldHourValue = oldHourValue + ", " + "Start Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime + " - " + "End Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime;
                    }
                }
                ...
                if (newHourCount > 0)
                {
                    newHourValue = "Start Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime + " - " + "End Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(0).StartTime;

                    for (int i = 1; i < oldHourCount; i++)
                    {
                        newHourValue = newHourValue + ", " + "Start Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime + " - " + "End Time : " + oldData.PracticeDays.ElementAt(0).DailyHours.ElementAt(i).StartTime;
                    }
                }
                ...
                {
                    ProfileUpdatedData day = new ProfileUpdatedData();
                    day.FieldName = oldData.PracticeDays.ElementAt(j).DayName;
                    day.OldValue = oldHourValue;
                    day.NewValue = newHourValue;
                    updatedList.Add(day);
                }
            ...
            if (oldData.AnyTimePhoneCoverage != newData.AnyTimePhoneCoverage)
            {
                ProfileUpdatedData hour = new ProfileUpdatedData();
                hour.FieldName = "24/7 Phone Coverage ?";
                hour.OldValue = oldData.AnyTimePhoneCoverage;
                hour.NewValue = newData.AnyTimePhoneCoverage;
                updatedList.Add(hour);
            }

            if (oldData.AnsweringService != newData.AnsweringService)
            {
                ProfileUpdatedData hour = new ProfileUpdatedData();
                hour.FieldName = "AnsweringService";
                hour.OldValue = oldData.AnsweringService;
                hour.NewValue = newData.AnsweringService;
                updatedList.Add(hour);
            }
            if (oldData.VoiceMailOther != newData.VoiceMailOther)
            {
                ProfileUpdatedData hour = new ProfileUpdatedData();
                hour.FieldName = "Voice Mail With Instructions To Call Answering Service";
                hour.OldValue = oldData.VoiceMailOther;
                hour.NewValue = newData.VoiceMailOther;
                updatedList.Add(hour);
            }
            if (oldData.VoiceMailToAnsweringService != newData.VoiceMailToAnsweringService)
            {
                ProfileUpdatedData hour = new ProfileUpdatedData();
                hour.FieldName = "Voice Mail With Other Instructions";
                hour.OldValue = oldData.VoiceMailToAnsweringService;
                hour.NewValue = newData.VoiceMailToAnsweringService;
                updatedList.Add(hour);
            }
            if (oldData.AfterHoursTelephoneNumber != newData.AfterHoursTelephoneNumber)
            {
                ProfileUpdatedData hour = new ProfileUpdatedData();
                hour.FieldName = "After Hours Telephone Number";
                hour.OldValue = oldData.AfterHoursTelephoneNumber;
        ...
        {
            List<ProfileUpdatedData> updatedList = new List<ProfileUpdatedData>();
            OpenPracticeStatus oldData = new JavaScriptSerializer().Deserialize<OpenPracticeStatus>(oldPracticeData);
        ...
        private List<ProfileUpdatedData> ConstructChangesForCoveringCollegues(dynamic oldCollegueData, dynamic newCollegueData)
        {
            ...
            PracticeProvider data = new JavaScriptSerializer().Deserialize<PersonalDetail>(oldCollegueData);
            ...
            int oldTypeCount = oldData.PracticeProviderSpecialties.Count;
            int newTypeCount = newData.PracticeProviderSpecialties.Count;
            string oldTypeValue = "";
            if (oldTypeCount > 0)
            {
