...
namespace AHC.CD.Business
{
    internal class ProfileManager : IProfileManager
    {
        ...
        public async Task<int> AddStateLicenseAsync(int profileId, Entities.MasterProfile.IdentificationAndLicenses.StateLicenseInformation stateLicense, DocumentDTO document)
        {
            try
            {
                if (profileRepository.Any(p => p.StateLicenses.Any(s => s.LicenseNumber.Equals(stateLicense.LicenseNumber) && !s.StateLicenseStatus.Status.Equals(AHC.CD.Entities.MasterData.Enums.StatusType.Inactive.ToString()))))
                {
        ...
        public async Task<FederalDEAInformation> RemoveFederalDEALicenseAsync(int profileId, FederalDEAInformation federalDEALicense)
        {
            try
            {
                //Save record into History
        ...
        public async Task<int> AddCVAsync(int profileId, CVInformation cvInformation, DocumentDTO document)
        {
            try
            {
                ...
                return cvInformation.CVInformationID;
            }
            catch (ApplicationException)
            {
                throw;
            ...
            try
            {
                if (await profileRepository.AnyAsync(p => p.ProfileID == profileId &&
                    p.ProfessionalWorkExperiences.Any(w => (workGap.StartDate >= w.StartDate && workGap.StartDate <= w.EndDate) &&
                                                            ...
                                                            (workGap.Status.Equals(AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString())))))
                {
                    throw new ProfessionalWorkExperienceExistException(ExceptionMessage.PROFESSIONAL_WORK_EXPERIENCE_EXISTS_EXCEPTION);
                }
