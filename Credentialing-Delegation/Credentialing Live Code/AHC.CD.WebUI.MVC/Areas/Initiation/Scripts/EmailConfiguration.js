...
emailconfigurationapp.controller('emailconfigurationController', ['$scope', '$parse','toaster', '$filter','ngTableParams', function ($scope,$parse, toaster,$filter, ngTableParams) {
    ...
    $scope
    $scope.StateLicenseIntervals = [15, 30, 60, 90, 180];
    $scope.FederalDEAIntervals = [15, 30, 60, 90, 180];
    $scope.CDSIntervals = [15, 30, 60, 90, 180];
    $scope.HospitalPrivilegeIntervals = [15, 30, 60, 90, 180];
    $scope.WorkerCompensationIntervals = [15, 30, 60, 90, 180];
    $scope.SpecialityDetailIntervals = [15, 30, 60, 90, 180];
    $scope.ProfessionalLiabilityIntervals = [15, 30, 60, 90, 180];

    $scope.AddCustomInterval = function (arrayname,array) {
        var model = $parse(arrayname);
        array.push('36');
        model.assign($scope, array);
        $scope.$apply();
    }

    $scope.SaveCustomInterval = function (arrayname, array) {
        var model = $parse(arrayname);
        array.push('36');
        model.assign($scope, array);
        $scope.$apply();
