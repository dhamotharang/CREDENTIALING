...
function ($scope, $rootScope, $http, $filter, masterDataService, locationService, messageAlertEngine, profileUpdates, $timeout) {
    $scope.UpdateHistory = [];
    var obj = {
        UpdatedBy: 'lharris@accesshealthcarellc.net',
        UpdatedOn: '07-12-2016',
        UpdatedFields: {
            FirstName: 'Tulasidhar',
            LastName: 'Reddy',
            Gender: 'Male',
    ...
    for (var i = 0; i < 5; i++) {
        $scope.UpdateHistory.push(obj);
        $scope.UpdateHistory[i].UpdatedFieldNames = Object.keys(obj.UpdatedFields);
    }


    $scope.showUpdateHistory = false;
    ...
    $scope.updateOtherLegalName = function (Form_Div_Id, index) {
        $rootScope.ChangedObj = JSON.stringify($rootScope.TrackObjectChanges($rootScope.OldObject, $rootScope.tempObject));
        var $form = $("#" + Form_Div_Id).find("form");
        var myData = {};
        ResetFormForValidation($form);
        if ($form.valid()) {
