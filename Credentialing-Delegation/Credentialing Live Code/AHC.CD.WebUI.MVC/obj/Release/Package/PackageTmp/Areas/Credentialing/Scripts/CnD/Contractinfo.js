...
ContractInfo.run(function ($rootScope, $q, $http, GetallProfile) {
    ...
    for (var i = 0; i < $rootScope.randomsItems.length; i++) {
        ...
        $rootScope.randomsItems[i].PracticeLocation = $rootScope.randomsItems[i].FacilityName + "-" + $rootScope.randomsItems[i].FacilityStreet + "," + $rootScope.randomsItems[i].FacilityCity + "," + $rootScope.randomsItems[i].FacilityState + "," + $rootScope.randomsItems[i].FacilityCountry;
        if ($rootScope.randomsItems[i].PracticeLocation == "-,,,") {
            $rootScope.randomsItems[i].PracticeLocation = '';
        }
        temporaryData[i].PracticeLocation = temporaryData[i].FacilityName + "-" + temporaryData[i].FacilityStreet + "," + temporaryData[i].FacilityCity + "," + temporaryData[i].FacilityState + "," + temporaryData[i].FacilityCountry;
...
ContractInfo.controller("ContractinfoController", function ($scope, $window, $rootScope, $filter, $timeout, $http, ngToast, Resource, GetallContract, GetallProfile, Max_Size, ContractGridID) {
    ...
    $scope.$watch("ProviderName", function (newValue, oldValue) {
        if (newValue === oldValue) {
            ...
            if (newValue == "") {
                ...
                for (var i in $rootScope.randomsItems) {
                    $rootScope.randomsItems[i].PracticeLocation = $rootScope.randomsItems[i].FacilityName + "-" + $rootScope.randomsItems[i].FacilityStreet + "," + $rootScope.randomsItems[i].FacilityCity + "," + $rootScope.randomsItems[i].FacilityState + "," + $rootScope.randomsItems[i].FacilityCountry;
                    if ($rootScope.randomsItems[i].PracticeLocation == "-,,,") {
                        $rootScope.randomsItems[i].PracticeLocation = '';
                    }
    ...
    $scope.viewContractGrid = function (ContractGridID, PlanName, LOBName) {
        ...
        $scope.tempNameObject.PracticeLocation = $scope.tempNameObject.FacilityName + "-" + $scope.tempNameObject.FacilityStreet + "," + $scope.tempNameObject.FacilityCity + "," + $scope.tempNameObject.FacilityState + "," + $scope.tempNameObject.FacilityCountry;
        if ($scope.tempNameObject.PracticeLocation == "-,,,") {
            $scope.tempNameObject.PracticeLocation = '';
        }
