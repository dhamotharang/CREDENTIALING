...
prototypeApp.controller('PrototypesCtrl', ["$scope", "$rootScope", "$http", "$q", "$filter", "ngTableParams", "Resource", "messageAlertEngine", function ($scope, $rootScope, $http, $q, $filter, ngTableParams, Resource, messageAlertEngine) {
    ...
    $scope.ccoList = [];
    $scope.tempObject = [];
    $scope.searchBtnCCO = false;
    $scope.searchBtnProviderSummary = false;
    $scope.FromDate = ""; //For CCO Summary
    $scope.ToDate = ""; //For CCO Summary
    ...
    //To clear the input box on clicking of Reset for CCO Summary
    $scope.clearSearchCCOSummary = function () {
        $scope.FromDate = "";
        $scope.ToDate = "";
        $scope.searchBtnCCO = false;
    };

    //To Clear the input box on clicking of Reset for Provider Summary
    $scope.clearSearchProviderSummary = function () {
        $scope.FromDateProviderSummary = "";
        $scope.ToDateProviderSummary = "";
        $scope.searchBtnProviderSummary = false;
    };

    //Check if From and To dates are empty for CCO Summary
    $scope.checkDates = function () {
        if ($scope.FromDate == "" || $scope.ToDate == "" || $scope.FromDate == undefined || $scope.ToDate == undefined)
            $scope.searchBtnCCO = false;
        else
            $scope.searchBtnCCO = true;

    }

    //Check if From and To dates are empty for Provider Summary
    $scope.checkDatesProviderSummary = function () {
        if ($scope.FromDateProviderSummary == "" || $scope.ToDateProviderSummary == "" || $scope.FromDateProviderSummary == undefined || $scope.ToDateProviderSummary == undefined)
            $scope.searchBtnProviderSummary = false;
        else
            $scope.searchBtnProviderSummary = true;

    }

    //To search CCO Summary for Date Range
    $scope.searchCCO = function () {
        GetCCOSummary().then(function (results) {
            $rootScope.PrototypeData = results.data;
            var tableState = {
                sort: {},
    ...
    //To search Provider Summary for Date Range
    $scope.searchProviderSummary = function () {
        GetProviderSummary().then(function (results) {
            $rootScope.PrototypeData = results.data;
            var tableState = {
                sort: {},
...
$(document).ready(function () {
    ...
    $(window).on('resize', function () {
        ...
        //check on resizing, the % of screen is greater than 50%
        if (((currentWindowWidth) * 100 / windowWidth) > 50) {
            $('#modalContent').addClass('modal-responsive');            
        }
        else {
