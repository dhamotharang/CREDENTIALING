...
masterDataGroup.controller('masterDataGroupController', ['$scope', '$http', '$filter', '$rootScope', 'messageAlertEngine', function ($scope, $http, $filter, $rootScope, messageAlertEngine) {
    ...
    $http.get(rootDir + "/MasterDataNew/GetAllGroups").then(function (value) {
        for (var i = 0; i < value.data.length ; i++) {
            if (value.data[i].Description == null)
            {
    ...
    $scope.saveGroup = function (idx) {
        ...
        var addData = {
            ...
            TaxId: $scope.tempGroup.TaxId,
            NPINumber: $scope.tempGroup.NPINumber,
            StatusType: 1
        }
        ...
        for (var i = 0; i < $scope.Groups.length; i++) {
            ...
            if (addData.Name && $scope.Groups[i].Name.replace(/\s+/g, "").toLowerCase() == addData.Name.replace(/\s+/g, "").toLowerCase()) {
    ...
    $scope.updateGroup = function (idx) {
        ...
        for (var i = 0; i < $scope.Groups.length; i++) {
            ...
            if (updateData.Name && $scope.Groups[i].GroupID != updateData.GroupID && $scope.Groups[i].Name.replace(/\s+/g, "").toLowerCase() == updateData.Name.replace(/\s+/g, "").toLowerCase()) {

                isExist = false;
                $scope.existErr = "The Name is Exist";
                break;
            }
            if ( $scope.Groups[i].GroupID == updateData.GroupID) {
                idx = i;
            }
