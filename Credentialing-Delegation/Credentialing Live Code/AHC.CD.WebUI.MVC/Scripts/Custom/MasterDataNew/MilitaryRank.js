...
masterDataMilitaryRanks.controller('masterDataMilitaryRanksController', ['$scope', '$http', '$filter', 'messageAlertEngine', function ($scope, $http, $filter, messageAlertEngine) {
    ...
    $scope.saveMilitaryRanks = function (idx) {
        //if ($scope.tempMilitaryRanks.MilitaryBranchID == 0) { }
        var addData = {
            MilitaryRankID: 0,            
        ...
        if ($scope.tempMilitaryRanks.MilitaryBranchTitle=="")
        {
            $scope.rankbranchError = "Please select the Branch";
            $scope.brancherror = true;
       }
        if (addData.Title && isExist && !$scope.brancherror)
        {
    ...
    $scope.UpdateMilitaryRanks = function (idx) {
        ...
        if (!updateData.Title) { $scope.rankError = "Please enter the Rank Title" }

        for (var i = 0; i < $scope.MilitaryRanks.length; i++) {
            if (updateData.Title && $scope.MilitaryRanks[i].InsuranceCarrierID != updateData.InsuranceCarrierID && $scope.MilitaryRanks[i].Title.replace(" ", "").toLowerCase() == updateData.Title.replace(" ", "").toLowerCase()) {
