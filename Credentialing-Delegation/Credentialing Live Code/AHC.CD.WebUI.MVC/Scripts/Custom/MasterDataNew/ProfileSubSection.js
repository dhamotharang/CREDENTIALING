...
MasterDataProfileSubsection.controller('profilesubsectionccontroller', ['$scope', '$http', '$filter', '$rootScope', 'masterDataService', 'messageAlertEngine', function ($scope, $http, $filter, $rootScope, masterDataService, messageAlertEngine) {
    ...
    $scope.updateProfileSubSections = function (idx) {
        ...
        for (var i = 0; i < $scope.ProfileSubSections.length; i++) {
            ...
            if ($scope.ProfileSubSections[i].ProfileSubSectionId == updateData.ProfileSubSectionId) {
                idx=i;
            }
        }
        if (!updateData.SubSectionName) { $scope.error = "Please Enter the Subsection Name"; }



        if (updateData.SubSectionName && isExist) {
            $http.post(rootDir + '/MasterDataNew/UpdateProfileSubSection', updateData).
                success(function (data, status, headers, config) {
                    try {
                        //----------- success message -----------
                        if (data.status == "true") {
