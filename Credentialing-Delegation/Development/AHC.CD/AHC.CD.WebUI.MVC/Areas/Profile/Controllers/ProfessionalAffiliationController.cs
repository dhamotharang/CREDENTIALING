...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Controllers
{
    public class ProfessionalAffiliationController : Controller
    {
        ...
        public async Task<ActionResult> UpdateProfessionalAffiliation(int profileId, ProfessionalAffiliationDetailViewModel professionalAffiliation,int CDUserID=0)
        {
            string status = "true";
            ProfessionalAffiliationInfo dataModelProfessionalAffiliation = null;
            bool isCCO = await GetUserRole();
            try
            {
                ...
                if (true)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        if (professionalAffiliation.UpdateHistory != "{}")
                        {
                            string ChangedObject = null;
                            string UserAuthID = null;
                            //string UserName = null;
                            string SectionName = "ProfessionalAffiliationInfoID";
                            if (CDUserID == 0)
                            {
                            ...
                            int ProfessionalAffiliationInfoID = 0;
                            if (professionalAffiliation.ProfessionalAffiliationInfoID != null)
                            {
                                ProfessionalAffiliationInfoID = (int)professionalAffiliation.ProfessionalAffiliationInfoID;
                            }
                            await profileManager.SaveUpdateHistory(professionalAffiliation.UpdateHistory, UserAuthID, SectionName, ProfessionalAffiliationInfoID, profileId);
                        }
                        #endregion
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Professional Affiliation Details", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
