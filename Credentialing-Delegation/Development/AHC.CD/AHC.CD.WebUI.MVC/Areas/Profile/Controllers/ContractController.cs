...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Controllers
{
    public class ContractController : Controller
    {
        ...
        public async Task<ActionResult> UpdateContractInformation(int profileId, AHC.CD.WebUI.MVC.Areas.Profile.Models.ContractInformation.ContractInfoViewModel contractInfo, int CDUserID = 0)
        {
            string status = "true";
            ContractInfo dataModelContractInfo = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        DocumentDTO document = CreateDocument(contractInfo.ContractFile);
                        if (contractInfo.ContractFilePath != null && contractInfo.ContractFilePath != null)
                        {
                            dynamic UH = JsonConvert.DeserializeObject(contractInfo.UpdateHistory);
                            UH.SupportingDocument = contractInfo.ContractFilePath;
                            contractInfo.UpdateHistory = JsonConvert.SerializeObject(UH);
                        }
                        await profileManager.UpdateContractInformationAsync(profileId, dataModelContractInfo, document);
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Contract Details", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(contractInfo.UpdateHistory, UserAuthID, "ContractInfoID", contractInfo.ContractInfoID, profileId);
                    }
                    else
                    {
        ...
        [HttpPost]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateContractGroupInformation(int contractInfoId, int profileId, ContractGroupInfoViewModel contractGroupInfo, int CDUserID = 0)
        {
            string status = "true";
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Contract Details - Group Information", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(contractGroupInfo.UpdateHistory, UserAuthID, "PracticingGroupId", contractGroupInfo.PracticingGroupId, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
