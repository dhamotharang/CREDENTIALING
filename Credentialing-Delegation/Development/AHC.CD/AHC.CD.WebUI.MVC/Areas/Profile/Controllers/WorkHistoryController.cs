...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Controllers
{
    public class WorkHistoryController : Controller
    {
        ...
        public async Task<ActionResult> UpdateProfessionalWorkExperienceAsync(int profileId, ProfessionalWorkExperienceViewModel professionalWorkExperience, int CDUserID = 0)
        {
            string status = "true";
            ProfessionalWorkExperience dataModelProfessionalWorkExperience = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(professionalWorkExperience.UpdateHistory, UserAuthID, "ProfessionalWorkExperienceID", professionalWorkExperience.ProfessionalWorkExperienceID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
        ...
        public async Task<ActionResult> RemoveProfessionalWorkExperienceAsync(int profileId, ProfessionalWorkExperienceViewModel professionalWorkExperience)
        {
            ...
            try
            {
                ...
                await profileManager.RemoveProfessionalWorkExperienceAsync(profileId, dataModelProfessionalWorkExperience,UserAuthID);
            ...
            return Json(new { status = status, professionalWorkExperience = dataModelProfessionalWorkExperience, UserName = UserName }, JsonRequestBehavior.AllowGet);
        }
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateMilitaryServiceInformationAsync(int profileId, MilitaryServiceInformationViewModel militaryServiceInformation, int CDUserID = 0)
        {
            string status = "true";
            MilitaryServiceInformation dataModelMilitaryServiceInformation = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(militaryServiceInformation.UpdateHistory, UserAuthID, "MilitaryServiceInformationID", militaryServiceInformation.MilitaryServiceInformationID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
        ...
        public async Task<ActionResult> RemoveMilitaryServiceInformationAsync(int profileId, MilitaryServiceInformationViewModel militaryServiceInformation)
        {
            ...
            try
            {
                ...
                await profileManager.RemoveMilitaryServiceInformationAsync(profileId, dataModelMilitaryServiceInformation,UserAuthID);
            ...
            return Json(new { status = status, militaryServiceInformation = dataModelMilitaryServiceInformation, UserName = UserName }, JsonRequestBehavior.AllowGet);
        }
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdatePublicHealthServiceAsync(int profileId, PublicHealthServiceViewModel publicHealthService, int CDUserID = 0)
        {
            string status = "true";
            PublicHealthService dataModelPublicHealthService = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(publicHealthService.UpdateHistory, UserAuthID, "PublicHealthServiceID", publicHealthService.PublicHealthServiceID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
        ...
        public async Task<ActionResult> RemovePublicHealthServiceAsync(int profileId, PublicHealthServiceViewModel publicHealthService)
        {
            ...
            try
            {
                ...
                await profileManager.RemovePublicHealthServiceAsync(profileId, dataModelPublicHealthService,UserAuthID);
            ...
            return Json(new { status = status, publicHealthService = dataModelPublicHealthService, UserName = UserName }, JsonRequestBehavior.AllowGet);
        }
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateWorkGapAsync(int profileId, WorkGapViewModel workGap, int CDUserID = 0)
        {
            string status = "true";
            WorkGap dataModelWorkGap = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(workGap.UpdateHistory, UserAuthID, "WorkGapID", workGap.WorkGapID, profileId);
                    }
                    else
                    {
                        await profileManager.CheckWorkGapUpdates(profileId, dataModelWorkGap);
                        string userId = await GetUserAuthId();
                        ProfileUpdateTrackerBusinessModel tracker = new ProfileUpdateTrackerBusinessModel();
        ...
        public async Task<ActionResult> RemoveWorkGap(int profileId, WorkGapViewModel workGap)
        {
            ...
            try
            {
                ...
                await profileManager.RemoveWorkGapAsync(profileId, dataModelWorkGap,UserAuthID);
            ...
            return Json(new { status = status, workGap = dataModelWorkGap, UserName = UserName }, JsonRequestBehavior.AllowGet);
        }
