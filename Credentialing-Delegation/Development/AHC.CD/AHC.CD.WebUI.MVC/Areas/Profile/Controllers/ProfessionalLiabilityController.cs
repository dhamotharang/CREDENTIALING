...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Controllers
{
    public class ProfessionalLiabilityController : Controller
    {
        ...
        public async Task<ActionResult> UpdateProfessionalLiabilityAsync(int profileId, AHC.CD.WebUI.MVC.Areas.Profile.Models.ProfessionalLiability.ProfessionalLiabilityInfoViewModel professionalLiability,int CDUserID=0)
        {
            string status = "true";
            ProfessionalLiabilityInfo dataModelProfessionalLiability = null;
            bool isCCO = await GetUserRole();
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    DocumentDTO document = CreateDocument(professionalLiability.InsuranceCertificateFile);
                    if (professionalLiability.InsuranceCertificateFile != null && professionalLiability.InsuranceCertificatePath != null)
                    {
                        dynamic UH = JsonConvert.DeserializeObject(professionalLiability.UpdateHistory);
                        UH.SupportingDocument = dataModelProfessionalLiability.InsuranceCertificatePath;
                        professionalLiability.UpdateHistory = JsonConvert.SerializeObject(UH);
                    ...
                    if (isCCO)
                    {
                        ...
                        if (professionalLiability.UpdateHistory != "{}")
                        {
                            string ChangedObject = null;
                            string UserAuthID = null;
                            //string UserName = null;
                            string SectionName = "ProfessionalLiabilityInfoID";
                            if (CDUserID == 0)
                            {
                            ...
                            int ProfessionalLiabilityInfoID = 0;
                            if (professionalLiability.ProfessionalLiabilityInfoID != null)
                            {
                                ProfessionalLiabilityInfoID = (int)professionalLiability.ProfessionalLiabilityInfoID;
                            }
                            await profileManager.SaveUpdateHistory(professionalLiability.UpdateHistory, UserAuthID, SectionName, ProfessionalLiabilityInfoID, profileId);
                        }
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Professional Liability Details", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                    }
        ...
        [HttpPost]
        public async Task<ActionResult> RenewProfessionalLiabilityAsync(int profileId, AHC.CD.WebUI.MVC.Areas.Profile.Models.ProfessionalLiability.ProfessionalLiabilityInfoViewModel professionalLiability, int CDUserID = 0)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Professional Liability Details", "Renewed");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(professionalLiability.UpdateHistory, UserAuthID, "ProfessionalLiabilityInfoID", professionalLiability.ProfessionalLiabilityInfoID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
