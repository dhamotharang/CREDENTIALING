...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Controllers
{
    public class DisclosureQuestionController : Controller
    {
        ...
        public string FileUpload(object obj)
        {
            ...
            string path = "\\ApplicationDocuments\\" + UniqueKeyGenerator.GetUniqueKey() + "AttachedFIle." + FileExtenstion.LastOrDefault();
            //Thread.Sleep(2000);
        ...
        [HttpPost]
        public async Task<ActionResult> UpdateDisclosureQuestionAsync(int profileId, ProfileDisclosureViewModel disclosureQuestion,int CDUserID=0)
        {
            string status = "true";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    else if (isCCO && disclosureQuestion.ProfileDisclosureID != 0)
                    {
                        ...
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Disclosure Questions", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(disclosureQuestion.UpdateHistory, UserAuthID, "ProfileDisclosureID", disclosureQuestion.ProfileDisclosureID, profileId);
                    }
                    else if (!isCCO && disclosureQuestion.ProfileDisclosureID != 0)
                    {
