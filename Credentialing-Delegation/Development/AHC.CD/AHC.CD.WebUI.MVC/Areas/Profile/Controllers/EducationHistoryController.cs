...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Controllers
{
    public class EducationHistoryController : Controller
    {
        ...
        public async Task<ActionResult> UpdateEducationDetailAsync(int profileId, EducationDetailViewModel educationDetails, int CDUserID = 0)
        {
            string status = "true";
            EducationDetail education = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (educationDetails.CertificateDocumentFile != null && educationDetails.CertificateDocumentFile != null)
                    {
                        dynamic UH = JsonConvert.DeserializeObject(educationDetails.UpdateHistory);
                        UH.SupportingDocument = educationDetails.CertificatePath;
                        educationDetails.UpdateHistory = JsonConvert.SerializeObject(UH);
                    }
                    ...
                    if (isCCO)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(educationDetails.UpdateHistory, UserAuthID, "EducationDetailID", educationDetails.EducationDetailID, profileId);
                    }
                    else
                    {
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateECFMGDetailAsync(int profileId, ECFMGDetailViewModel ecfmgDetails, int CDUserID = 0)
        {
            string status = "true";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (ecfmgDetails.ECFMGCertificateDocumentFile != null && ecfmgDetails.ECFMGCertificateDocumentFile != null)
                    {
                        dynamic UH = JsonConvert.DeserializeObject(ecfmgDetails.UpdateHistory);
                        UH.SupportingDocument = ecfmgDetails.ECFMGCertPath;
                        ecfmgDetails.UpdateHistory = JsonConvert.SerializeObject(UH);
                    }
                    ...
                    if (ecfmgDetails.ECFMGDetailID == 0)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsyncForAdd(notification, isCCO);
                        await profileManager.SaveUpdateHistory(ecfmgDetails.UpdateHistory, UserAuthID, "ECFMGDetailID", ecfmgDetails.ECFMGDetailID, profileId);
                    }
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateCMECertificationAsync(int profileId, CMECertificationViewModel CMEDetails, int CDUserID = 0)
        {
            string status = "true";
            CMECertification CME = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Education History - Post Graduate Training - CME Details", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(CMEDetails.UpdateHistory, UserAuthID, "CMECertificationID", CMEDetails.CMECertificationID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateProgramDetailAsync(int profileId, ProgramDetailViewModel programDetails, int CDUserID = 0)
        {
            string status = "true";
            ProgramDetail program = null;
            string UserAuthID = null;
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Education History - Residency/Internship/Fellowship Details", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await profileManager.SaveUpdateHistory(programDetails.UpdateHistory, UserAuthID, "ProgramDetailID", programDetails.ProgramDetailID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
