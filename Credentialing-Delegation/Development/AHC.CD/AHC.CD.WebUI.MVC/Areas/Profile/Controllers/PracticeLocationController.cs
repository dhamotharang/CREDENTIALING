...
namespace AHC.CD.WebUI.MVC.Areas.Profile.Controllers
{
    public class PracticeLocationController : Controller
    {
        ...
        public PracticeLocationController(IPracticeLocationManager practiceLocationManager, IOrganizationManager organizationManager, IErrorLogger errorLogger, IChangeNotificationManager notificationManager, IProfileUpdateManager profileUpdateManager, IProfileManager iProfileManager)
        {
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Add, false)]
        public async Task<ActionResult> updatePracticeLocationDetailInformaton(int profileId, PracticeLocationDetailViewModel practiceLocationDetailViewModel, int CDUserID = 0)
        {
            string status = "true";
            PracticeLocationDetail dataModelPracticeLocationDetailInformation = null;
            bool isCCO = await GetUserRole();
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        {                            
                            ...
                            ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Practice Locations Details", "Updated");
                            await notificationManager.SaveNotificationDetailAsync(notification);
                            await ProfileManager.SaveUpdateHistory(practiceLocationDetailViewModel.UpdateHistory, UserAuthID, "PracticeLocationDetailID", practiceLocationDetailViewModel.PracticeLocationDetailID, profileId);
                        }
                        else
                            throw new Exception();
                    }
        ...
        public async Task<ActionResult> UpdateOpenPracticeStatusAsync(int profileId, OpenPracticeStatusViewModel openPracticeStatus, int CDUserID = 0)
        {
            string status = "true";
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    else if (isCCO && openPracticeStatus.OpenPracticeStatusID != 0)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await ProfileManager.SaveUpdateHistory(openPracticeStatus.UpdateHistory, UserAuthID, "OpenPracticeStatusID", openPracticeStatus.OpenPracticeStatusID, profileId);
                    }
                    else if (!isCCO && openPracticeStatus.OpenPracticeStatusID != 0)
                    {
                        string userId = await GetUserAuthId();
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdatePracticeProviderAsync(int profileId, PracticeProviderViewModel practiceProvider, int CDUserID = 0)
        {
            string status = "true";
            PracticeProvider dataModelPracticeProvider = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (isCCO)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await ProfileManager.SaveUpdateHistory(practiceProvider.UpdateHistory, UserAuthID, "PracticeProviderID", practiceProvider.PracticeProviderID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> AddOfficeManagerAsync(int profileId, FacilityEmployeeViewModel officemanager, int CDUserID = 0)
        {
            string status = "true";
            ...
            try
            {
                ...
                else if (isCCO && officemanager.EmployeeID != 0)
                {
                    ...
                    await notificationManager.SaveNotificationDetailAsync(notification);
                    await ProfileManager.SaveUpdateHistory(officemanager.UpdateHistory, UserAuthID, "PracticeLocationDetailID", officemanager.PracticeLocationDetailID, profileId);
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> AddBillingContactAsync(int profileId, FacilityEmployeeViewModel billingcontact, int CDUserID = 0)
        {
            string status = "true";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    else if (isCCO && billingcontact.EmployeeID != 0)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await ProfileManager.SaveUpdateHistory(billingcontact.UpdateHistory, UserAuthID, "PracticeLocationDetailID", billingcontact.PracticeLocationDetailID, profileId);
                    }
                    else if (!isCCO && billingcontact.EmployeeID != 0)
                    {
                        string userId = await GetUserAuthId();
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> AddPaymentAndRemittanceAsync(int profileId, PracticePaymentAndRemittanceViewModel paymentandremittance,int CDUserID=0)
        {
            string status = "true";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    else if (isCCO && paymentandremittance.PracticePaymentAndRemittanceID != 0)
                    {
                        ...
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await ProfileManager.SaveUpdateHistory(paymentandremittance.UpdateHistory, UserAuthID, "PracticePaymentAndRemittanceID", paymentandremittance.PracticePaymentAndRemittanceID, profileId);
                    }
                    else if (!isCCO && paymentandremittance.PracticePaymentAndRemittanceID != 0)
                    {
                        string userId = await GetUserAuthId();
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateCredentialingContact(int profileId, FacilityEmployeeViewModel credentialingContact, int CDUserID = 0)
        {
            string status = "true";
            Employee dataModelCredentialingContact = null;
            bool isCCO = await GetUserRole();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        await practiceLocationManager.UpdatePrimaryCredentialingContactAsync(credentialingContact.PracticeLocationDetailID, dataModelCredentialingContact);
                        await ProfileManager.SaveUpdateHistory(credentialingContact.UpdateHistory, UserAuthID, "PracticeLocationDetailID", credentialingContact.PracticeLocationDetailID, profileId);
                    }
                    else
                    {
                        string userId = await GetUserAuthId();
        ...
        [AjaxAction]
        [ProfileAuthorize(ProfileActionType.Edit, false)]
        public async Task<ActionResult> UpdateWorkersCompensationInformationAsync(int profileId, WorkersCompensationInfoViewModel workersCompensationInformation, int CDUserID = 0)
        {
            string status = "true";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    else if (isCCO && workersCompensationInformation.WorkersCompensationInformationID != 0)
                    {
                        ...
                        ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Practice Locations - Workers Compensation Details", "Updated");
                        await notificationManager.SaveNotificationDetailAsync(notification);
                        await ProfileManager.SaveUpdateHistory(workersCompensationInformation.UpdateHistory, UserAuthID, "WorkersCompensationInformationID", workersCompensationInformation.WorkersCompensationInformationID, profileId);
                    }
                    else if (!isCCO && workersCompensationInformation.WorkersCompensationInformationID != 0)
                    {
                        string userId = await GetUserAuthId();
