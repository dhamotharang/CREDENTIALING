...
profileApp.controller('ProfessionalReference', ['$scope', '$rootScope', '$http', 'masterDataService', 'locationService', 'messageAlertEngine', '$filter', 'profileUpdates', function ($scope, $rootScope, $http, masterDataService, locationService, messageAlertEngine, $filter, profileUpdates) {
  ...
  */
    ...
    $scope.ConstructOldandNewObject = function (providerType, tempSpecialty) {
        var oldObject = angular.copy($rootScope.OldObject);
        var newObject = angular.copy($scope.tempObject);
        oldObject.ProviderType != null ? oldObject.ProviderTypeID = oldObject.ProviderType.Title : null; oldObject.ProviderType = null;
        newObject.ProviderType != null ? newObject.ProviderTypeID = providerType.Title : newObject.ProviderType = null;
        oldObject.Specialty != null ? oldObject.SpecialtyID = oldObject.Specialty.Name : null; oldObject.Specialty = null;
        newObject.Specialty != null ? newObject.SpecialtyID = tempSpecialty.Name : newObject.Specialty = null;
        oldObject.BoardCerifiedOption = oldObject.BoardCerifiedOption != null ? (oldObject.BoardCerifiedOption == 1 ? "Yes" : "No") : null;
        newObject.BoardCerifiedOption = newObject.BoardCerifiedOption != null ? (newObject.BoardCerifiedOption == 1 ? "Yes" : "No") : null;
    ...
    $scope.saveProfessionalReference = function (professionalReference, index) {
        ...
        if ($scope.visibilityControl == 'addpr') {
            ...
            var result = $scope.ConstructOldandNewObject(providerType, tempSpecialty);
            $('#UpdateHistoryHiddenProfessionalReference').val(JSON.stringify($rootScope.TrackObjectChanges(result.oldObject, result.newObject)));
