/// <reference path="ContractGrid.js" />

profileApp.directive('searchdropdown', function () {
    return {
        restrict: 'AE',
...
profileApp.controller('singleProviderCtrl', function ($scope, $http, $q, $timeout, $location, $filter, $rootScope, messageAlertEngine) {
    ...
    //    $("#" + divId).show();
    //};
    $rootScope.FlipingTabs = function (i, j) {
        var id_show = '#' + i;
        var id_hide = '#' + j;
        $(id_show).show();
        $(id_hide).hide();
        $scope.new_search(1);
        $scope.SelectProviderForCredentialingInitiation();
    };
    ...
    $scope.addIntoPlanDropDown = function (type, tempObject, div) {
        ...
        $scope.tempObject.PlanId = type.PlanID;
        $scope.tempObject.PlanName = type.PlanName;
    ...
    $scope.new_search = function (id) {
        ...
        //$scope.changeTabs('GenInfoTab', 'id01', 'GenInfo');
        $scope.localdata = JSON.parse(localStorage.getItem('dataToStore'));
        if (localStorage.getItem('NavigateToInitiateRecredential') == 'true') {
            $scope.tempObject.NPINumber = 3221863804;
            $scope.tempObject.FirstName = 'DSXK';
