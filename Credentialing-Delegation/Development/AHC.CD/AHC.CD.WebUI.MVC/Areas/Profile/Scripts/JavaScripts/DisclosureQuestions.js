...
//profileApp.filter('CustomFilter', function () {
...
profileApp.controller('DisclosureController', ['$scope', '$rootScope', '$http', 'masterDataService', 'messageAlertEngine', 'profileUpdates', function ($scope, $rootScope, $http, masterDataService, messageAlertEngine, profileUpdates) {
	...
	$scope.CancelTemp=function(data){
		...
		$scope.disclosureQuestions=angular.copy(data);
	}
    ...
    $rootScope.$on("LoadRequireMasterDataDisclosureQuestion", function () {
        ...
        masterDataService.getMasterData(rootDir + "/Profile/MasterData/getAllQuestions").then(function (masterQuestions) {
            ...
            if ($scope.masterQuestions.length != 0) {
                $scope.masterQuestions = $scope.masterQuestions.filter(function (masterQuestions) { return masterQuestions.QuestionCategoryId != null })
                for (i in $scope.masterQuestions) {
                    $scope.masterQuestions[i].TableStatus = false;
                }
            }
            $rootScope.tempMasterQuestions = angular.copy($scope.masterQuestions);
        });
    ...
    $scope.saveQuestions = function (disclosureQuestions, type) {
        ...
        $('#UpdateHistoryForDisclosureQuestion').val(JSON.stringify($rootScope.TrackChangesForRadioOptions($rootScope.OldObject, $rootScope.tempObject)));
    ...
    $rootScope.$on('DisclosureQuestion', function () {
        if (!$scope.dataLoaded) {
            ...
            $http({
                ...
                console.log(data);
                if (data != null) {
                    $rootScope.tempObject = data.ProfileDisclosure;
                }
                else {
                    $rootScope.tempObject = data;
                }
                //$rootScope.tempObject = data.ProfileDisclosure;
                console.log($rootScope.tempObject);
                try {
                    for (key in data) {
