...
function ($scope, $rootScope, $http, $filter, masterDataService, locationService, messageAlertEngine, profileUpdates, $timeout) {
    ...
    $rootScope.$on('PersonalDetail', function (event, val) {
        ...
        $rootScope.FirstName = $scope.Provider.PersonalDetails.FirstName;
        $rootScope.MiddleName = $scope.Provider.PersonalDetails.MiddleName == null ? "" : $scope.Provider.PersonalDetails.MiddleName;
    ...
    $scope.DemographicsUpdatePersonalDetails = function (Form_Id) {
        var copynewobj = angular.copy($scope.TempPersonalDetailsForEdit);
        var copyoldobj = angular.copy($scope.Provider.PersonalDetails);
        var newProviderTitles = angular.copy(copynewobj.ProviderTitles);
        var oldProviderTitles = angular.copy(copyoldobj.ProviderTitles);
        copynewobj.ProviderTitles = "";
        copyoldobj.ProviderTitles = "";
        var changedTitlesObj = "";
        jQuery.grep(newProviderTitles, function (newele) { jQuery.grep(oldProviderTitles, function (oldele) { newele.ProviderTitleID == oldele.ProviderTitleID ? changedTitlesObj += changedTitlesObj == "" ? oldele.ProviderType.Title : ", " + oldele.ProviderType.Title : null }); });
        var changedobj = $rootScope.TrackObjectChanges(copyoldobj, copynewobj);
        changedobj["Provider Titles"] = changedTitlesObj;
        $('#UpdateHistoryHidden').val(JSON.stringify(changedobj));
        ResetFormForValidation($("#" + Form_Id));
                ...
                success: function (data) {
                    ...
                    try {
                        if (data.status == "true") {
                            ...
                            if ($scope.Provider.OtherLegalNames.length > 0) {
                                $scope.OtherLegalNamesPendingRequest = true;
                            }
    ...
    $scope.updateHomeAddress = function (Form_Div_Id, index) {
        ...
        delete oldObject.IsPresentlyStaying; delete newObject.IsPresentlyStaying;
        oldObject.IsPresentlyStayingYesNoOption = oldObject.IsPresentlyStayingYesNoOption != null ? (oldObject.IsPresentlyStayingYesNoOption == 1 ? "Yes" : "No") : null;
        newObject.IsPresentlyStayingYesNoOption = newObject.IsPresentlyStayingYesNoOption != null ? (newObject.IsPresentlyStayingYesNoOption == 1 ? "Yes" : "No") : null;
    ...
    $scope.ContactDetailsToggle = function (condition) {
        if (condition == 'EditContactDetails') {
            ...
            $scope.tempPcIndex = $scope.pcIndex;
            $rootScope.UpdateHistorySectionVisibility = "";
        } else {
            $rootScope.visibilityControl = condition;
            $scope.ContactDetailsEmptyError = false;
            $scope.pcIndex = $scope.tempPcIndex;
        }
    };
    ...
    $scope.TrackContactDetailChanges = function (oldObject, newObject, nestedObjectbool, isEmail) {
        ...
        for (var key1 in oldObject) {
            for (var key2 in newObject) {
                if (key1 == key2 && oldObject[key1] != newObject[key1] && typeof newObject[key1] != "object") {
                    if (!isEmail)
                        switch (newObject["PhoneType"]) {
                            case "Home":
                                returnchangedObj["Home Phone"] = returnchangedObj["Home Phone"] == undefined ? oldObject["CountryCode"] + "-" + oldObject["Number"] : returnchangedObj["Home Phone"] + ", " + oldObject["CountryCode"] + "-" + oldObject["Number"];
                                break;
                            case "Fax":
                                returnchangedObj["Home Fax"] = returnchangedObj["Home Fax"] == undefined ? oldObject["CountryCode"] + "-" + oldObject["Number"] : returnchangedObj["Home Fax"] + ", " + oldObject["CountryCode"] + "-" + oldObject["Number"];
                                break;
                            case "Mobile":
                                returnchangedObj["Mobile Number"] = returnchangedObj["Mobile Number"] == undefined ? oldObject["CountryCode"] + "-" + oldObject["Number"] : returnchangedObj["Mobile Number"] + ", " + oldObject["CountryCode"] + "-" + oldObject["Number"];
                                break;
                        }
                    else {
                        if (key2 == "EmailAddress")
                            returnchangedObj["Email Id"] = returnchangedObj["Email Id"] == undefined ? oldObject[key1] : returnchangedObj["Email Id"] + ", " + oldObject[key1];
                    }
                }
                else if (key1 == key2 && oldObject[key1] != newObject[key1] && typeof newObject[key1] == "object") {
                    $scope.TrackContactDetailChanges(oldObject[key1], newObject[key1], true, isEmail);
                };
            }
        }
    ...
    $scope.saveContactDetails = function (Form_Id) {
        var changedPhoneDetailsobj = {};
        var changedEmailIdsobj = {};
        var personalPagerNumber = {};
        if ($scope.Provider.ContactDetails.PhoneDetails.length != 0) {
            changedPhoneDetailsobj = $scope.TrackContactDetailChanges($scope.Provider.ContactDetails.PhoneDetails, $scope.TempContactDetailsForEdit.PhoneDetails, false, false);
        }
        if ($scope.Provider.ContactDetails.EmailIDs.length != 0) {
            changedEmailIdsobj = $scope.TrackContactDetailChanges($scope.Provider.ContactDetails.EmailIDs, $scope.TempContactDetailsForEdit.EmailIDs, false, true);
        }
        if ($scope.Provider.ContactDetails.PagerNumber != null && $scope.TempContactDetailsForEdit.PagerNumber != null && $scope.Provider.ContactDetails.PagerNumber != $scope.TempContactDetailsForEdit.PagerNumber){
            personalPagerNumber = { "Pager Number": $scope.Provider.ContactDetails.CountryCode + "-" + $scope.Provider.ContactDetails.PagerNumber };
        }
        $('#UpdateHistoryHidden').val(JSON.stringify(jQuery.extend(changedPhoneDetailsobj, changedEmailIdsobj, personalPagerNumber)));
        ResetFormForValidation($("#" + Form_Id));
    ...
    $scope.DemographicsPersonalIdentificationSave = function (Form_Id) {
        $('#UpdateHistoryHidden').val(JSON.stringify($rootScope.TrackObjectChanges($scope.Provider.PersonalIdentification, $rootScope.tempObject)));
        var validCount = 0;
        var url;
        var $formData;
    ...
    $scope.saveBirthInformation = function (Form_Id) {
        $('#UpdateHistoryHidden').val(JSON.stringify($rootScope.TrackObjectChanges($scope.Provider.BirthInformation, $rootScope.tempObject)));
        ResetFormForValidation($("#" + Form_Id));
        var myData = {};
        if ($("#" + Form_Id).valid()) {
            $.ajax({
    ...
    $scope.saveEthnicityVisaDetails = function (Form_Id) {
        var oldObject = angular.copy($scope.Provider.VisaDetails);
        var newObject = angular.copy($rootScope.tempObject);
        if(oldObject!=null){
        oldObject.VisaInfo["VisaTypeID"] = jQuery.grep($scope.VisaTypes, function (ele) { return ele.VisaTypeID == oldObject.VisaInfo.VisaTypeID; })[0].Title;
        oldObject.VisaInfo["VisaStatusID"] = jQuery.grep($scope.VisaStatuses, function (ele) { return ele.VisaStatusID == oldObject.VisaInfo.VisaStatusID; })[0].Title;
        }
        if(newObject!=null){
        newObject.VisaInfo["VisaTypeID"] = jQuery.grep($scope.VisaTypes, function (ele) { return ele.VisaTypeID == newObject.VisaInfo.VisaTypeID; })[0].Title;
        newObject.VisaInfo["VisaStatusID"] = jQuery.grep($scope.VisaStatuses, function (ele) { return ele.VisaStatusID == newObject.VisaInfo.VisaStatusID; })[0].Title;
        }
    ...
    $scope.saveLanguages = function (Form_Id) {
        ...
        var oldObject = angular.copy($scope.Provider.LanguageInfo.KnownLanguages);
        }
        $('#UpdateHistoryHidden').val(JSON.stringify(oldObject));
        ResetFormForValidation($("#" + Form_Id));
        if ($("#" + Form_Id).valid()) {
            $.ajax({
