...
profileApp.controller('professionalAppCtrl', ['$scope', '$rootScope', '$http', 'messageAlertEngine', '$filter', 'profileUpdates', function ($scope, $rootScope, $http, messageAlertEngine, $filter, profileUpdates) {
        ...
        if ($scope.affiliationHistoryArray.length == 0) {
            ...
            $http.get(historuUrl).success(function (data) {
                try {
                    ...
                    for (var j = 0; j < $scope.affiliationHistoryArray.length; j++) {
                        for (var i = 0; i < $rootScope.userslist.length; i++) {
                            if ($scope.affiliationHistoryArray[j].DeletedById != null) {
                                if ($rootScope.userslist[i].CDUserID == $scope.affiliationHistoryArray[j].DeletedById) {
                                    if ($rootScope.userslist[i].FullName != null) {
                                        $scope.affiliationHistoryArray[j].DeletedBy = $rootScope.userslist[i].FullName;
                                        break;
                                    }
                                    else {
                                        $scope.affiliationHistoryArray[j].DeletedBy = $rootScope.userslist[i].UserName;
                                        break;
                                    }
                                }
                            }
                        }
                        if ($scope.affiliationHistoryArray[j].DeletedDate != null) {
                            var date = moment.utc($scope.affiliationHistoryArray[j].DeletedDate).toDate();
                            $scope.affiliationHistoryArray[j].DeletedDate = moment(date).format('MM/DD/YYYY, h:mm:ss a');
                        }
                    }
    ...
    $scope.saveProfessionalAffiliation = function (professionalAffiliation, index) {
        ...
        if ($scope.visibilityControl == 'addpa') {
            ...
            $('#UpdateHistoryHiddenForPA').val(JSON.stringify($rootScope.TrackObjectChanges($rootScope.OldObject, $rootScope.tempObject)));
