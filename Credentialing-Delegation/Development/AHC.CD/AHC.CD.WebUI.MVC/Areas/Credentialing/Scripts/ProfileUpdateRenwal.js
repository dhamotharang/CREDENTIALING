...
initCredApp.controller('listController', ['$scope', '$http', '$timeout', 'messageAlertEngine', 'ngTableParams', '$filter', '$q', function ($scope, $http, $timeout, messageAlertEngine, ngTableParams, $filter, $q) {
    ...
    $scope.UpdateSuccess = function (data) {
        $scope.flag = true;
        var dataasobj = JSON.parse(data.NewData);
        var UpdateHistory = dataasobj.UpdateHistory;
        ...
        var newData1 = JSON.parse(data.NewData);
        newData1.UpdateHistory = UpdateHistory;
        data.NewData = JSON.stringify(newData1);

        $.ajax({
            type: 'POST',
            url: rootDir + data.Url + data.ProfileId + "&CDUserID=" + data.LastModifiedBy,
