...
var providerApp = angular.module("providerApp", ['mgcrea.ngStrap', 'ahc.cd.autosearch', 'debounce']);
//
...
providerApp.controller("providerController", ['$scope','$timeout', '$http', 'locationService', function ($scope, $timeout, $http, locationService) {
    ...
    //-------------------------View the selected CV document in preview-------------
    $scope.PreviewTheCVDocument = function () {
        //$scope.fileObject = document.getElementById("docInput" + index).files[0];
        $scope.fileObject = $scope.selectedCV;
        if ($scope.fileObject) {
            $scope.thisIsFileURL = URL.createObjectURL($scope.fileObject);
            window.open($scope.thisIsFileURL);
        }
    }

    //-------------------------View the selected Contract document in preview-------

    $scope.PreviewTheDocument = function () {
        //$scope.fileObject = document.getElementById("docInput" + index).files[0];
        $scope.fileObject = $scope.selectedContract;
        if ($scope.fileObject) {
            $scope.thisIsFileURL = URL.createObjectURL($scope.fileObject);
            window.open($scope.thisIsFileURL);
        }
	...
	$scope.IsValid = function (fieldName) {
	    if (fieldName == "OtherIdentificationNumber_NPINumber") {
	        if ($("#" + fieldName).valid() && $("#" + fieldName).val()) {
	            ...
	            $scope.GetProviderInfo($("#" + fieldName).val());
    ...
    $scope.GetProviderInfo = function(npiNumber){
        
        $http.get('/Initiation/Provider/GetProviderDatafromNPIRegistry', { params: { key: "number", value: npiNumber } }).success(function (data) {
            try {
                console.log("Provider Info");
                console.log(data.results[0]);                
                $scope.FormatProviderInfo(data.results[0])
                
            }
            catch (e) { $scope.CancelProviderInfo(); }

        });
    }

    $scope.CancelProviderInfo = function () {
        $scope.PersonalDetail = {
            "NPI_Number": "",
            "SalutationType": 0,
            "FirstName": "",
    ...
    $scope.FormatProviderInfo = function (providerInfo) {
        ...
        $scope.HomeAddresses = {};
        try {
            if (providerInfo.basic.credential) {
                providerType = $scope.ProviderTypes.filter(function (ProviderType) { return ProviderType.Code == providerInfo.basic.credential })[0];
                if (providerType)
                    providerTitles.push({ "ProviderTypeId": providerType.ProviderTypeID, "ProviderType": providerType, "StatusType": 1 })
            }
        }
        catch (e) { }        

        $scope.PersonalDetail = {
            "NPI_Number": providerInfo.number,
            "SalutationType":0,
            "FirstName": providerInfo.basic.first_name,            
            "LastName": providerInfo.basic.last_name,            
            "GenderType": providerInfo.basic.gender == 'M' ? 1 : 2,            
            "ProviderTitles": providerTitles
        }
        try{
            $scope.HomeAddresses = {
                "Street": providerInfo.addresses[1].address_1,
                "UnitNumber": providerInfo.addresses[1].address_2,
                "City": providerInfo.addresses[1].city,
                "State": providerInfo.addresses[1].state,
                "ZipCode": providerInfo.addresses[1].postal_code,
                "Country": providerInfo.addresses[1].country_name,
                "County": "",            
            }

            if (providerInfo.addresses[1].telephone_number != '') {
                phoneNumber = providerInfo.addresses[1].telephone_number.split("-")[0] + providerInfo.addresses[1].telephone_number.split("-")[1] + providerInfo.addresses[1].telephone_number.split("-")[2];
                $scope.ContactDetail = {
                    "EmailAddress": "",
                    "CountryCode": "+1",
                    ...
                    "PreferenceType": 1,
                    "StatusType": 1,
                    "PreferredWrittenContactType1": 3,
                    "StatusType1": 1,
                    "PreferredIndex1": 1,
                    "PreferredWrittenContactType2": 4,
                    "StatusType2": 1,
                    "PreferredIndex2": 2
                }
            }
                ...
                $scope.ContactDetail = {
                    ...
                    "PreferenceType": 1,
                    "StatusType": 1,
                    "PreferredWrittenContactType1": 4,
                    "StatusType1": 1,
                    "PreferredIndex1": 1
                }
            }
