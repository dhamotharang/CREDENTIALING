...
EmailServiceApp.filter("CheckGroupName", ["$rootScope", function ($rootScope) {
    return function (group) {
        if ($rootScope.activeGroups.filter(function (ActiveGroups) { return ActiveGroups.EmailGroupName.toLowerCase() == group.toLowerCase() }).length == 0 && $rootScope.inactiveGroups.filter(function (InactiveGroups) { return InactiveGroups.EmailGroupName.toLowerCase() == group.toLowerCase() }).length == 0) {
            return false;
        }
        else {
            return true;
...
EmailServiceApp.controller("GroupEmailController", function ($rootScope, $timeout, $scope, $http, $filter, $q, Resource1, messageAlertEngine, ngTableParams, toaster) {
    ...
    $scope.UpdateGroup = function () {
        ...
        if (!$scope.grpNameErrorMessage && !$scope.descriptionErrorMessage && $rootScope.tempObject.EmailObj.length != 0 && !$scope.GroupNameExists) {
               ...
               success(function (data, status, headers, config) {
                   try {
                       if (data.status == "true") {
                           ...
                           //ctrl.temp = $rootScope.groupMailList;
                           $scope.t = angular.copy($scope.resetTable());
                           ctrl.callServer($scope.t);
                           $rootScope.visibility = 'groupMailList';
    ...
    $scope.CheckGroupNameifExists = function (grpname) {
        if (!$rootScope.isEdit) {
            $scope.GroupNameExists = $filter('CheckGroupName')(grpname);
        }
        else {
            $scope.GroupNameExists = grpname.toLowerCase() == $rootScope.tempEditGroupName.toLowerCase() ? false : $filter('CheckGroupName')(grpname);
        }
    }
    ...
    $scope.pushEmail = function (data) {
        ...
        if ((data.hasOwnProperty("UserType"))) {
            if (data.UserType == "Provider") {
                ...
                if (angular.isDefined($rootScope.searchDataVal.filter(function (names) { return names.CDuserId == data.CDuserId })[0])) {
                ...
                if (angular.isDefined($rootScope.UserDataResult.filter(function (names) { return names.CDuserId == data.CDuserId })[0])) {
    ...
    $scope.removeSelected = function (data) {
        ...
        try {
            ...
            if (angular.isDefined($rootScope.filtered.filter(function (names) { return names.CDuserId == data.CDuserId })[0])) {
    ...
    $scope.List = [{ id: 1, name: "Doctor" }, { id: 2, name: "Mid - Level" }, { id: 3, name: "Others" }]
            ...
            for (var i = 0; i < selected.length; i++) {
                switch (selected[i].split(" ")[0]) {
                    case "Provider":
                        ...
                        data = angular.copy($rootScope.selectedProviderData);
                        break;
                    case "IPA":
                        if (data.length == 0) {
                            ...
                            $rootScope.selectedIPAData.push($filter('filter')(data, selected[i].split(" ")[1]));
                        }
                        $rootScope.selectedIPAData = Array.prototype.concat.apply([], $rootScope.selectedIPAData);
                        data = angular.copy($rootScope.selectedIPAData);
                        break;
                    case "Relationship":
                        if (data.length == 0) {
                            ...
                            $rootScope.selectedRelationshipData.push($filter('filter')(data, selected[i].split(" ")[1]));
                        }
                        $rootScope.selectedRelationshipData = Array.prototype.concat.apply([], $rootScope.selectedRelationshipData);
                        data = angular.copy($rootScope.selectedRelationshipData);
                        break;
                }
            }
            $rootScope.groupMailList = angular.copy(data);
            $rootScope.searchResult = angular.copy(data);
            $scope.t = angular.copy($scope.resetTable());
            $scope.t.pagination.numberOfPages = 0;
            ctrl.displayed = [].concat(data);
            ctrl.callServer($scope.t);
        }
        else {
            if (!$rootScope.users) {
