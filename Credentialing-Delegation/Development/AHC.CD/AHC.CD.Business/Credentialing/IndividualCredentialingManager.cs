...
namespace AHC.CD.Business.Credentialing
{
    internal class IndividualCredentialingManager : IIndividualCredentialingManager
    {
                    ...
                    if (item.CredentialingLogs.Any(s => s.Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.Credentialing.ToString()) && !(item.CredentialingLogs.Any(s => s.Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.DeCredentialing.ToString())))
                    {
                        foreach (var data in item.CredentialingLogs)
                        {
                            if (data.Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.Credentialing.ToString())
                            {
                                ...
                                if (!(data.CredentialingActivityLogs.Any(x => x.Activity == AHC.CD.Entities.MasterData.Enums.ActivityType.Closure.ToString() && x.ActivityStatus == AHC.CD.Entities.MasterData.Enums.ActivityStatusType.Completed.ToString())))
                                {
                                    credList.Add(item);
                                    break;
                                }
            ...
            try
            {
                    ...
                    for (var i = 0; i < CredentialingContractRequestsArray.Length; i++)
                    {
                        foreach (var data in credentialingInitiationConReqList1)
                        {
                            if (data.CredentialingContractRequestID == CredentialingContractRequestsArray[i])
                            {
                                ...
                                data.ContractGrid.ToList().ForEach(x => x.ContractGridStatusType = AHC.CD.Entities.MasterData.Enums.StatusType.Inactive);
                ...
                foreach (var data1 in credentialingInitiationConReqList)
                {
                    ...
                    foreach (var data2 in data1.ContractGrid)
                    {
                        ...
                        data2.Report.ReCredentialingDate = null;
                        data2.Report.InitiatedDate = null;
                    }
                }
        ...
        public int InitiateDeCredentialingAsync(int credentialingInfoID, int contractRequestId, int gridId, string userAuthId, int reasonId, string documentDTO)
        {
            var credentialingInitiationInfoRepo = uow.GetGenericRepository<CredentialingInfo>();
            ...
            try
            {
                ...
                if (credInfo != null)
                {
                    //credInfo.DecredentialingReasonId = reasonId;
                    //credInfo.DecredentialingReasonDocumentPath = documentDTO;
                        ...
                        if (b.CredentialingContractRequestID == contractRequestId)
                        {
                            foreach (var c in b.ContractGrid)
                            {
                                if (c.ContractGridID == gridId)
                                {
                                    ...
                                    c.ContractGridStatusType = AHC.CD.Entities.MasterData.Enums.StatusType.Inactive;
                                    c.DecredentialingReasonId = reasonId;
                                    c.DecredentialingReasonDocPath = documentDTO;
                                }
        ...
        //                                {
        //                                    if (data6.Report.ReCredentialingDate != null && data6.Report.ReCredentialingDate < DateTime.Now)
        //                                    {
        //                                        counter2++;
        //                                    }
        //                                }
        ...
        //                                    {
        //                                        ApprovStatus++;
        //                                        if (data7.Report.ReCredentialingDate != null && data7.Report.ReCredentialingDate > DateTime.Now)
        //                                        {
        //                                            ApprovStatuswithTermination++;
        //                                        }
        //                                        if (data7.Report.ReCredentialingDate == null)
        //                                        {
        //                                            TerminateStatus = 1;
        //                                        }
        //                                    }
        ...
        public async Task<IEnumerable<Object>> getAllCredentialinginfoByContractRequest(int ProviderID, int PlanID)
        {
            try
            {
                ...
                List<CredentialingInfo> credentialingInfo1 = new List<CredentialingInfo>();
                List<Object> CredentialingContractRequestsReference = new List<Object>();
                //var credentialingInfoes=credentialingInfo.Where(x=>x.CredentialingContractRequests.)
                foreach (var data in credentialingInfo.Where(x => x.Status == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString()).ToList())
                {
                    if (data.ProfileID == ProviderID && data.PlanID == PlanID)
                    {
                        foreach (var data1 in data.CredentialingContractRequests)
                ...
                foreach (var data in credentialingInfo1)
                {

                    var ContractRequests = data.CredentialingContractRequests != null ? data.CredentialingContractRequests.Where(x => x.ContractGrid.Any(y => y.ContractGridStatus == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString() && y.Status == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString()) && x.ContractRequestStatus == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString() && x.Status == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString()).ToList() : null;
                    foreach (var Request in ContractRequests)
                    {
                        var tmp = new
                        {
                            TableRowStatus = true,
                            credID = data.CredentialingInfoID,
                            CredentialingContractRequests = Request,
                        };
                        CredentialingContractRequestsReference.Add(tmp);
                    }
        ...
        public async Task<IEnumerable<object>> getAllCredentialinginfoByContractGrid(int ProviderID, int PlanID)
        {
            ...
            //    }
            //}
            credentialingInfo1 = credentialingInfo.Where(x => x.ProfileID == ProviderID && x.PlanID == PlanID).ToList();
            Parallel.ForEach(credentialingInfo1, (credentialinginfo) =>
            {
                Parallel.ForEach(credentialinginfo.CredentialingContractRequests, (contractrequests) =>
                {
                    Parallel.ForEach(contractrequests.ContractGrid, (grid) =>
                    {
                        grid.CredentialingInfo = null;
                        if (grid.ContractGridStatus == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString())
                        {
                            var ContractGridTemporaryObject = new
                            {
                                CredInfoID = credentialinginfo.CredentialingInfoID,
                                CredRequestID = contractrequests.CredentialingContractRequestID,
                                ContractGridObject = grid
                            };
            ...
            //Parallel.ForEach(credentialingInfo1, (data1) =>
            ////foreach (var data1 in credentialingInfo1)
            //{
            //    Parallel.ForEach(data1.CredentialingContractRequests, (data2) =>
            //    //foreach (var data2 in data1.CredentialingContractRequests)
            //    {
            //        if (data2.Status == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString() && data2.ContractRequestStatus == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString())
            //        {
            //            Parallel.ForEach(data2.ContractGrid, (data3) =>
            //            //foreach (var data3 in data2.ContractGrid)
            //            {
            //                if (data3.Status == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString() && data3.Report != null)
            //                {
            //                    if (data3.Report.CredentialingApprovalStatus == AHC.CD.Entities.MasterData.Enums.ApprovalStatusType.Approved.ToString() && data3.Report.ReCredentialingDate != null && data3.Report.ReCredentialingDate > DateTime.Now)
            //                    {
            //                        var ContractGridTemporaryObject = new
            //                        {
