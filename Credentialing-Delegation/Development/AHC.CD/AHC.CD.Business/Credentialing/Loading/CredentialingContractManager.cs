...
namespace AHC.CD.Business.Credentialing.Loading
{
    public class CredentialingContractManager : ICredentialingContractManager
    {
        ...
        ICredentialingContractRepository credentialingContractRepository = null;

        public CredentialingContractManager(IUnitOfWork uow, IDocumentsManager documentManager, ICredentialingContractRepository credentialingContractRepository)
        {
            this.uow = uow;
            ...
            this.credentialingContractRepository = credentialingContractRepository;
        }
                    ...
                    foreach (var contractLOB in credentialingContractRequest.ContractLOBs)
                    {
                        foreach (var contractPracticeLocation in credentialingContractRequest.ContractPracticeLocations)
                        {
                            foreach (var contractSpecialty in credentialingContractRequest.ContractSpecialties)
                            {
                                ...
                                addContractGrid.ContractGridStatusType = Entities.MasterData.Enums.StatusType.Active;
        ...
        public async Task<List<ContractGrid>> GetContractGridForCredentialingInfo(int credentialingInfoID)
        {
            try
            {
                ...
                foreach (var item in contractGrid)
                {
                    ...
                    item.CredentialingInfo.Plan.PlanLOBs.ToList().ForEach(x => { x.Plan = null; x.LOB = null; });
        ...
        public async Task<IEnumerable<AHC.CD.Data.ADO.DTO.ContractGridDTO>> GetAllContractGridDTO()
        {
            try
            {
                var contractGridList = await credentialingContractRepository.GetAllContractGrid();
        ...
        public async Task<IEnumerable<AHC.CD.Data.ADO.DTO.ContractGridDTO>> GetAllInactiveContractGrid()
        {
            try
            {
                var contractGridList = await credentialingContractRepository.GetAllInactiveContractGrid();
                return contractGridList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ContractGrid> AddContractInfoFromPlan(ContractGrid contractGrid, DocumentDTO welcomeLetter, string authId, string ReasonForPanelChange)
        {
            try
            {
                ...
                ContractGrid updateContractGrid = await contractGridRepo.FindAsync(c => c.ContractGridID == contractGrid.ContractGridID, "ProfileSpecialty.Specialty, ProfilePracticeLocation.Facility, LOB, CredentialingInfo, CredentialingInfo.Plan.PlanLOBs, CredentialingInfo.Profile, BusinessEntity, Report, CredentialingInfo.CredentialingLogs.CredentialingActivityLogs, ProfilePracticeLocation.BusinessOfficeManagerOrStaff, ContractGridHistory");
                if (contractGrid.Report.ParticipatingStatus == "TERMED")
                {
                    updateContractGrid.ContractGridStatusType = StatusType.Inactive;
                }
                ...
                updateContractGrid.CredentialingInfo.Plan.PlanLOBs.ToList().ForEach(x => { x.LOB = null; x.Plan = null; });
        ...
        public async Task<ContractGrid> QuickSaveContractInfoFromPlan(ContractGrid contractGrid, string authId)
        {
            try
            {
                int userId = GetUserId(authId);
                var contractGridRepo = uow.GetGenericRepository<ContractGrid>();
                ContractGrid updateContractGrid = await contractGridRepo.FindAsync(c => c.ContractGridID == contractGrid.ContractGridID, "ProfileSpecialty.Specialty, ProfilePracticeLocation.Facility, LOB, CredentialingInfo, CredentialingInfo.Plan.PlanLOBs, CredentialingInfo.Profile, BusinessEntity, Report, CredentialingInfo.CredentialingLogs.CredentialingActivityLogs");
        ...
        /// <returns></returns>
        public async Task<int> DeactivateGrid(int contractGridID)
        {
            try
            {
                var contractGridRepo = uow.GetGenericRepository<ContractGrid>();
                ContractGrid inactivateContractGrid = await contractGridRepo.FindAsync(c => c.ContractGridID == contractGridID, "Report");
                inactivateContractGrid.ContractGridStatusType = Entities.MasterData.Enums.StatusType.Inactive;
        ...
        public async Task<int> RemoveLoadToPlan(int contractGridID)
        {
            try
            {
                var contractGridRepo = uow.GetGenericRepository<ContractGrid>();
        ...
        public async Task<int> ReactivateGrid(int contractGridID)
        {
            try
            {
                var contractGridRepo = uow.GetGenericRepository<ContractGrid>();
                ContractGrid activateContractGrid = await contractGridRepo.FindAsync(c => c.ContractGridID == contractGridID, "Report");
                activateContractGrid.ContractGridStatusType = Entities.MasterData.Enums.StatusType.Active;
                contractGridRepo.Update(activateContractGrid);
                contractGridRepo.Save();
                return activateContractGrid.ContractGridID;
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public Task<IEnumerable<AHC.CD.Entities.Credentialing.ParticipatingStatusType>> GetAllParticipatingStatus()
        {
            try
            {
                var ParticipatingStatusTypeRepo = uow.GetGenericRepository<ParticipatingStatusType>();
                var ParticipatingStatusList = ParticipatingStatusTypeRepo.GetAllAsync();
                return ParticipatingStatusList;
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public async Task<ContractGrid> GetContractGridById(int ContractGridID)
        {
            ContractGrid ContractGrid = null;
            try
            {
                var ContractGridRepo = uow.GetGenericRepository<ContractGrid>();
                ContractGrid = await ContractGridRepo.FindAsync(p => p.ContractGridID == ContractGridID, "CredentialingInfo,CredentialingInfo.Plan.PlanLOBs,Report");
                ContractGrid.CredentialingInfo.Plan.PlanLOBs.ToList().ForEach(x => { x.Plan = null; x.LOB = null; });
            }
            catch (Exception ex)
            {
        ...
        public async Task<CredentialingContractInfoFromPlan> ViewContractInfoFromPlan(int ReportID)
        {
            CredentialingContractInfoFromPlan credentialingContractInfoFromPlan = new CredentialingContractInfoFromPlan();
            try
            {
                credentialingContractInfoFromPlan = await uow.GetGenericRepository<CredentialingContractInfoFromPlan>().FindAsync(x => x.CredentialingContractInfoFromPlanID == ReportID);
            }
            catch(Exception ex)
            {
                throw ex;
            }
            return credentialingContractInfoFromPlan;
        }
    }
}
