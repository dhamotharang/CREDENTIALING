...
namespace AHC.CD.Business.Credentialing.PSVInfo
{
    public interface IPSVManager
    {
        ...
        int? UpdateVerifiedData(int credVerificationInfoId, int profileId, ProfileVerificationDetail verificationData, DocumentDTO verificationDocument, string userAuthId, string DocumentPath);
        List<ProfileVerificationDetail> GetPSVHistory(int profileId);
        ProfileVerificationInfo GetPSVDetailsForAProvider(int profileId);
    }
}
