    ...
    internal class PSVManager : IPSVManager
    {
        ...
        public List<Entities.Credentialing.Loading.CredentialingInfo> GetAllPSVList()
        {
            try
            {
                ...
                var psvData = psvRepo.Get(p => p.IsDelegated.Equals(Entities.MasterData.Enums.YesNoOption.YES.ToString()) && p.Status != Entities.MasterData.Enums.StatusType.Inactive.ToString() && p.Profile.Status == Entities.MasterData.Enums.StatusType.Active.ToString(), "Plan, Profile.PersonalDetail.ProviderTitles.ProviderType,CredentialingLogs,CredentialingLogs.CredentialingActivityLogs").ToList();
        ...
        public int? UpdateVerifiedData(int VerificationInfoId, int profileId, ProfileVerificationDetail verificationData, DocumentDTO verificationDocument, string userAuthId,string DocumentPath)
        {
            try
            {
                var userRepo = uow.GetGenericRepository<CDUser>();
                ...
                {
                    verificationData.VerificationResult.VerificationDocumentPath = DocumentPath;
                }
