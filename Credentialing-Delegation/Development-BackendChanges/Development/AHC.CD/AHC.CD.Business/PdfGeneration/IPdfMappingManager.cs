...
namespace AHC.CD.Business.PdfGeneration
{
    public interface IPdfMappingManager
    {
        ...
        string GeneratePlanFormPDF(int profileId, string templateName, string UserAuthId);
        void AddDocument(int profileID,string documentTitle);
        string CombinePdfs(int profileID, List<string> pdflist, string UserAuthId);
        List<PackageGeneratorBusinessModel> GenerateBulkPackage(List<int> ProfileIDs, List<string> GenricList, string UserAuthId);
        List<PlanFormGenerationBusinessModel> GenerateBulkForm(List<int> ProfileIDs, List<string> TemplateName, string UserAuthId);
    }
}
