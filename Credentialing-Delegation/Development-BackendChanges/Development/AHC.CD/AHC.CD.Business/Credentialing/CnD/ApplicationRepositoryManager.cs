...
{
    internal class ApplicationRepositoryManager : IApplicationRepositoryManager
    {
       private IProfileRepository profileRepository = null;
        private IUnitOfWork uow = null;

        public ApplicationRepositoryManager(IUnitOfWork uow)
        {
            this.profileRepository = uow.GetProfileRepository();
            this.uow = uow;
        }
        public async Task<string> GetProfileDataByIdAsync(int profileId, string template)
        {
            try
            {
                ...
                string primaryPDFName = ConstructProfilePrimaryData(profile, template);
                ...
                primaryPDFName = pdf.CombineApplicationFormPdfs(pdfList, fileName);
        ...
        private string ConstructProfilePrimaryData(Profile profile, string template)
        {
            string pdfName = "";            
            ...
            try
            {
                ...
                var path = pdfCall.FillApplicationForm(profileData, pdfName, template);
        ...
        private List<string> ConstructSuplementProfessionaIDs(Profile profile)
        {
            ...
            try
            {
                ...
                for (int i = 1; i < largestCount; i++)
                {
                    ...
                    name = pdfCall.FillApplicationForm(suplement, pdfName, "IDs_new.pdf");
            ...
            try
            {
                for (int i = 1; i < profile.ProgramDetails.Count; i++)
                {
                    ...
                    if (profile.ProgramDetails.Count > i)
                    {
                        ...
                        name = pdfCall.FillApplicationForm(training, pdfName, "Training_new.pdf");
            ...
            try
            {
                ...
                if (SpecialtyDetails.Count > 1)
                {
                    ...
                    for (int i = 1; i < SpecialtyDetails.Count; i++)
                    {
                        ...
                        name = pdfCall.FillApplicationForm(specialty, pdfName, "Specialty_new.pdf");
                ...
                if (profile.HospitalPrivilegeInformation != null)
                {
                    ...
                    if (SecondaryHospitalPrivilegeDetail.Count > 1)
                    {
                        ...
                        for (int i = 1; i < SecondaryHospitalPrivilegeDetail.Count; i++)
                        {
                            ...
                            name = pdfCall.FillApplicationForm(hospital, pdfName, "Hosp_new.pdf");
        ...
        private List<string> ConvertProfessionalLiabilityPDF(Profile profile)
        {
            ...
            try
            {
                for (int i = 2; i < profile.ProfessionalLiabilityInfoes.Count; i++)
                {
                    ...
                    name = pdfCall.FillApplicationForm(liability, pdfName, "Liability_new.pdf");
        ...
        private List<string> ConstructWorkHistoryPDF(Profile profile)
        {
            ...
            try
            {
                for (int i = 3; i < profile.ProfessionalWorkExperiences.Count; i++)
                {
                    ...
                    name = pdfCall.FillApplicationForm(history, pdfName, "WorkHistory_new.pdf");
        ...
        private List<string> ConstrustWorkGapsPdf(Profile profile)
        {
            ...
            try
            {
                for (int i = 1; i < profile.WorkGaps.Count; i++)
                {
                    ...
                    name = pdfCall.FillApplicationForm(workGaps, pdfName, "Gap_new.pdf");
                    gapName.Add(name);
                }
            ...
            try
            {
                ...
                if (primaryPracticeLocation != null)
                {
                    ...
                    for (int j = 3; j < coveringColleagues1.Count; j++)
                    {
                        ...
                        name = pdfCall.FillApplicationForm(colleague, pdfName, "Covering Collg_new.pdf");
            ...
            try
            {
                if (profile.ProfileDisclosure != null)
                {
                    ...
                    for (int i = 0; i < answerYes.Count; i++)
                    {
                        ...
                        name = pdfCall.FillApplicationForm(question, pdfName, "Qn_new.pdf");
            ...
            try
            {
                ...
                for (int i = 0; i < praticeInfo.Count; i++)
                {
                    if (praticeInfo.Count > i)
                    {
                        ...
                        name = pdfCall.FillApplicationForm(practice, pdfName, "PLoc_new.pdf");
