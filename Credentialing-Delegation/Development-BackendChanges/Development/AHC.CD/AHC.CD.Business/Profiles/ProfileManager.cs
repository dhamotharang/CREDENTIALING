...
namespace AHC.CD.Business
{
    internal class ProfileManager : IProfileManager
    {
        ...
        public async Task<SpecialtyDetail> UpdateSpecialtyDetailAsync(int profileId, SpecialtyDetail specialtyDetail, DocumentDTO document)
        {
            try
            {
                List<SpecialtyDetail> checkSpecialtyDetails = profileRepository.Find(p => p.ProfileID == profileId, "SpecialtyDetails").SpecialtyDetails.Where(s => s.SpecialtyDetailID != specialtyDetail.SpecialtyDetailID && s.SpecialtyPreference == PreferenceType.Primary.ToString()).ToList();
