...
initCredApp.controller('listController', ['$scope', '$http', '$timeout', 'messageAlertEngine', 'ngTableParams', '$filter', '$q', function ($scope, $http, $timeout, messageAlertEngine, ngTableParams, $filter, $q) {
    ...
    var getValue = function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].FieldName == 'DocumentCategory') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'Fax') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'Phone') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'Telephone') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'EmployerFax') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'EmployerMobile') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'InsuranceCoverageType') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'PrimaryTax') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'SpecialtyBoardNotCertifiedDetailID') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'SpecialtyBoardExamStatus') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            if (data[i].FieldName == 'MedicaidInformationID') {
                data.splice(data.indexOf(data[i]), 1);
                i--;
            }
            ...
            if ($scope.temp.Section == 'Practice Location' && $scope.temp.SubSection == 'Practice Location Detail') {
                if (data[i].FieldName == 'OpenPracticeStatus') {
                    data.splice(data.indexOf(data[i]), 1);
                }

                if (data[i].FieldName == 'WorkersCompensationInformation') {
                    data.splice(data.indexOf(data[i]), 1);
                }

                if (data[i].FieldName == 'BusinessOfficeManagerOrStaffId') {
                    data.splice(data.indexOf(data[i]), 1);
                }

                if (data[i].FieldName == 'PaymentAndRemittanceId') {
                    data.splice(data.indexOf(data[i]), 1);
                }
                if (data[i].FieldName == 'BillingContactPersonId') {
                    data.splice(data.indexOf(data[i]), 1);
                }
                if (data[i].FieldName == 'PrimaryCredentialingContactPersonId') {
                    data.splice(data.indexOf(data[i]), 1);
                }
                if (data[i].FieldName == 'OfficeHour') {
                    data.splice(data.indexOf(data[i]), 1);
                }
                if (data[i].FieldName == 'OfficeHourId') {
                    data.splice(data.indexOf(data[i]), 1);
                }
            }
        }
        ...
        $http.post(rootDir + "/Credentialing/ProfileUpdates/CredentialingRequestTrackerSetApproval", obj).
           success(function (data, status, headers, config) {
               ...
               if (data.status == "true") {
                   ...
                   $("#UpdateFailCredReq").hide();
                   $scope.hideApprovalBtn = true;
                   messageAlertEngine.callAlertMessage('successfulInitiated', "Credentialing Request Initiated Successfully. !!!!", "success", true);
                   var obj = {};
                   for (var i = 0; i < $scope.AllCredentilaingRequestData.length; i++) {
                       if ($scope.AllCredentilaingRequestData[i].CredentialingRequestID == requestdata.CredentialingRequestID) {
                           obj = $scope.AllCredentilaingRequestData[i];
