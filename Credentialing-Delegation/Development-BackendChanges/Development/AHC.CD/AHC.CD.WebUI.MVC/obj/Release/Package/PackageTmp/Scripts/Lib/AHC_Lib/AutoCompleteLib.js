...
ahc_cd_autosearch.directive('input', ['$compile', function ($compile) {
    return {
        ...
        link: function (scope, element, attr, ngModel) {
            ...
            else if (attr.autocomplete == "stateautocomplete") {
                element.bind('focus', function () {
                    element.parent().find(".ProviderTypeSelectAutoList").first().show();
                });
                var contentTr = angular.element(
                    ...
                    "<tbody><tr ng-repeat='state in filteredState = (States | filter:" + attr.ngModel + ")' ng-click='" + attr.ngModel + "= state; hideDiv();' class='pointer'>" +
                    "<td>{{state}}</td></tr><tr ng-if='States.length == 0'><td>No data..... </td></tr>"+
                    "<tr ng-if='States.length > 0 && filteredState.length == 0'><td>State not found..... </td></tr></tbody>"+
                    "</tbody></table></div>");
...
ahc_cd_autosearch.directive('midlevelpractitionersearch', ['$compile', function ($compile) {
    return {
        restrict: 'AE',
        link: function (scope, element, attr) {
            if (attr.autocomplete == "practitioner") {
                ...
                var contentTr = angular.element(
                    ...
                    "<td>{{practitioner.FirstName}} {{practitioner.LastName}} ({{practitioner.NPINumber}})</td></tr>" +
                    "<tr ng-if='practitioners.length<1'><td>No " + attr.msg + " Available</td></tr></tbody></table></div>");
