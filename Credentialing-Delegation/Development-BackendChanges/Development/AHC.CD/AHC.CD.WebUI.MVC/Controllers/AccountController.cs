...
namespace AHC.CD.WebUI.MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<JsonResult> ChangeRole(string RoleCode, string Email)
        {
            UsersList u = new UsersList();
            List<ICollection<CDUserRole>> pro = new List<ICollection<CDUserRole>>();
            try
            {
                var user = UserManager.FindByEmail(Email);
                var currentrole = user.Roles.First();
                user.Roles.Remove(currentrole);
                UserManager.AddToRole(user.Id, RoleCode);
                UserManager.Update(user);
                //pro = await UserManager.SearchUser(Email, RoleCode); 
                var result = UserManager.GetRoles(user.Id);
                var result1 = UserManager.GetEmail(user.Id);
                u.Name = user.FullName;
                u.Email = result1;
                u.Role = result.FirstOrDefault();
        ...
        //Akash //for passwprd Reset only by Admin
        [AllowAnonymous]
