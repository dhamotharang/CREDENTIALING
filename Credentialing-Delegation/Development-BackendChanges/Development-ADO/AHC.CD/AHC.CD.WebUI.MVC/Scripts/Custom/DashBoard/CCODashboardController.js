...
dashboardApp.controller("CCODashboardController", ["$scope", "$http", "$filter", "CCOLicenseService", function ($scope, $http, $filter, CCOLicenseService) {
    ...
    $scope.printData = function () {
        $scope.printDataById($scope.LicenseType);
    }
    ...
    $scope.printDataById = function (id) {
        ...
        mywindow.document.write('<center><b style="font-size:large">' + id + '</b></center>');
        ...
        mywindow.document.write('<style>.ng-hide:not(.ng-hide-animate) {display: none !important;} @page{size: auto;margin: 5mm;}</style>');
        mywindow.document.write('<style>th{text-align: center;}</style>');
        ...
        pdf.addHTML(mywindow.document.read(), function () {
