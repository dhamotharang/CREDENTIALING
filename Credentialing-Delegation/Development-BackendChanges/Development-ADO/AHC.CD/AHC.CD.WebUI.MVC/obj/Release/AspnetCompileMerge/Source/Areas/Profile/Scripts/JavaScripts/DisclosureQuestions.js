...
profileApp.controller('DisclosureController', ['$scope', '$rootScope','$http', 'masterDataService', 'messageAlertEngine', function ($scope, $rootScope, $http, masterDataService, messageAlertEngine) {
    ...
    $scope.setAllNo = function (obj) {
        $scope.isDataPresent = true;
        $('input:radio[class^=selectNo][value=2]').each(function (i) {
            this.checked = true;
        });
        for (var i = 0; i < obj.ProfileDisclosureQuestionAnswers.length; i++) {
            obj.ProfileDisclosureQuestionAnswers[i].AnswerYesNoOption = 2;
            obj.ProfileDisclosureQuestionAnswers[i].Reason = '';
