...
initCredApp.controller('listController', ['$scope', '$http', '$timeout', 'messageAlertEngine', 'ngTableParams', '$filter', '$q', function ($scope, $http, $timeout, messageAlertEngine, ngTableParams, $filter, $q) {
    ...
    $scope.pushdata = function (ProfileUpdatesTrackerId) {
        $scope.businessError = "";
        ...
        $scope.chnagedData = [];
        $http.post(rootDir + "/Credentialing/ProfileUpdates/GetDataById?trackerId=" + data.ProfileUpdatesTrackerId).then(function (value) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == "true") {
                    ...
                    $.ajax({
                        type: "POST",
                        url: rootDir + "/Credentialing/ProfileUpdates/SetApproval",
                        data: {
