        ...
        $http.get(rootDir + '/Credentialing/Initiation/getCredentialingContractRequest?ProviderID=' + ProviderID + '&PlanID=' + Plan).
            success(function (data, status, headers, config) {
                if (data.status == true) {
                    ...
                    for (var i = 0; i < $scope.Credinfo.length; i++) {
                        for (var j = 0; j < $scope.Credinfo[i].CredentialingContractRequests.length; j++) {
                            if ($scope.Credinfo[i].CredentialingContractRequests[j].Status == 'Active' && $scope.Credinfo[i].CredentialingContractRequests[j].ContractRequestStatus == 'Active') {
                                ...
                                var ContractGridReports = [];
                                for (var k = 0; k < $scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid.length; k++) {
                                    if ($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid[k].Report != null) {
                                        if ($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid[k].Report.CredentialingApprovalStatus == null) {
                                            ContractGridReports.push($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid[k]);
                                        }
                                    }
                                    else {
                                        ContractGridReports.push($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid[k]);
                                ...
                                for (var k = 0; k < ContractGridReports.length; k++) {
                                    $scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid.splice($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid.indexOf(ContractGridReports[k]), 1);
                                }
                                
                                var ContractGridReportsTermDate = [];
                                for (var k = 0; k < $scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid.length; k++) {
                                    
                                    if ($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid[k].Report.TerminationDate == null) {
                                        ContractGridReportsTermDate.push($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid[k]);
                                    }
                                }
                                for (var k = 0; k < ContractGridReportsTermDate.length; k++) {
                                    $scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid.splice($scope.Credinfo[i].CredentialingContractRequests[j].ContractGrid.indexOf(ContractGridReportsTermDate[k]), 1);
                                }
                                ...
                                if (ContractGridTableStatus == true) {
                                    $scope.Credinfo[i].CredentialingContractRequests[j].TableRowStatus = false;
                                    $scope.Credinfo[i].CredentialingContractRequests[j].credID = $scope.Credinfo[i].CredentialingInfoID;
                                    $scope.CredentialingContractRequests.push($scope.Credinfo[i].CredentialingContractRequests[j]);
                                }
                    ...
                    else if ($scope.CredentialingContractRequests.length>0) {
