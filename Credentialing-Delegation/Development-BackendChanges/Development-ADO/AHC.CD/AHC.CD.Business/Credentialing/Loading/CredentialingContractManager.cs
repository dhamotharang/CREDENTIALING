...
namespace AHC.CD.Business.Credentialing.Loading
{
    public class CredentialingContractManager : ICredentialingContractManager
    {
        ...
        public async Task<object> GetAllContractGrid()
        {
            try
            {
                var contractGridList = new List<ContractGrid>();
                var contractGridRepo = uow.GetGenericRepository<ContractGrid>();
                var contractGrid = await contractGridRepo.GetAsync(c => c.Status != StatusType.Inactive.ToString(), "Report, ProfileSpecialty.Specialty, ProfileSpecialty.SpecialtyBoardCertifiedDetail, ProfileSpecialty.SpecialtyBoardCertifiedDetail.SpecialtyBoard, ProfilePracticeLocation.Facility, ProfilePracticeLocation.BillingContactPerson, CredentialingInfo.Plan, CredentialingInfo.Profile, BusinessEntity, LOB, ProfilePracticeLocation.BusinessOfficeManagerOrStaff, CredentialingInfo.CredentialingLogs");
                
                foreach (var item in contractGrid)
                {
                    item.CredentialingInfo.CredentialingContractRequests = null;
                    if (item.CredentialingInfo.CredentialingLogs.Last().Credentialing != AHC.CD.Entities.MasterData.Enums.CredentialingType.Droped.ToString())
                    {
                        if (item.CredentialingInfo.CredentialingLogs.Last().Credentialing == AHC.CD.Entities.MasterData.Enums.CredentialingType.ReCredentialing.ToString())
                        {
                            foreach (var newItem in contractGridList)
                            {
                                if (newItem.CredentialingInfo.CredentialingInfoID > item.CredentialingInfo.CredentialingInfoID && newItem.CredentialingInfo.ProfileID == item.CredentialingInfo.ProfileID && newItem.CredentialingInfo.PlanID == item.CredentialingInfo.PlanID)
                                {
                                    if (newItem.BusinessEntityID == item.BusinessEntityID && newItem.LOBID == item.LOBID && newItem.ProfilePracticeLocationID == item.ProfilePracticeLocationID && newItem.ProfileSpecialtyID == item.ProfileSpecialtyID)
                                    {
                                        contractGridList.Remove(newItem);
                                        contractGridList.Add(item);
                                    }
                                }
                            }
                        }
        ...
        public async Task<ContractGrid> AddContractInfoFromPlan(ContractGrid contractGrid, DocumentDTO welcomeLetter, string authId)
        {
            try
            {
                ...
                var contractGridRepo = uow.GetGenericRepository<ContractGrid>();
                ContractGrid updateContractGrid = await contractGridRepo.FindAsync(c => c.ContractGridID == contractGrid.ContractGridID, "ProfileSpecialty.Specialty, ProfilePracticeLocation.Facility, LOB, CredentialingInfo, CredentialingInfo.Plan, CredentialingInfo.Profile, BusinessEntity, Report, CredentialingInfo.CredentialingLogs.CredentialingActivityLogs, ProfilePracticeLocation.BusinessOfficeManagerOrStaff");
