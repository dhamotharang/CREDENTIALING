            ...
            try
            {
               ...
               // int flag = 0;
                ...
                if (ProviderName.Length == 1)
                {
                    ...
                    profiles = profileRepository.Get(p => p.PersonalDetail != null && (p.PersonalDetail.FirstName.Contains(firstname) || p.PersonalDetail.LastName.Contains(firstname)) && p.Status != AHC.CD.Entities.MasterData.Enums.StatusType.Inactive.ToString(), includeProperties).ToList();
