    ...
    internal class ProfileUpdateManager : IProfileUpdateManager
    {
        ...
        public List<ProfileUpdatesTracker> GetUpdatesById(int profileId)
        {
            try
            {
                List<ProfileUpdatesTracker> updates = new List<ProfileUpdatesTracker>();
                var trackerRepo = uow.GetGenericRepository<ProfileUpdatesTracker>();
                var updatedData = trackerRepo.GetAll().Where(c => c.ProfileId == profileId && c.ApprovalStatusType != AHC.CD.Entities.MasterData.Enums.ApprovalStatusType.Approved && c.ApprovalStatusType != AHC.CD.Entities.MasterData.Enums.ApprovalStatusType.Rejected).ToList();
                return updatedData;
            }
            catch (Exception)
            {
