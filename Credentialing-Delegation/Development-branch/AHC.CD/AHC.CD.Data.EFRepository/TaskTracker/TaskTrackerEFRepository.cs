...
namespace AHC.CD.Data.EFRepository.TaskTracker
{
    internal class TaskTrackerEFRepository : ITaskTrackerRepository
    {
        ...
        public void ReactiveTask(int taskTrackerId,int taskID)
        {
            try
            {
                Entities.TaskTracker.TaskTracker task = taskRepo.Find(t => t.TaskTrackerId == taskTrackerId && t.Status == AHC.CD.Entities.MasterData.Enums.StatusType.Inactive.ToString());
        ...
        public Entities.TaskTracker.TaskTracker GetTaskById(int taskTrackerId)
        {
            try
            {
                ...
                var task = taskRepo.Find(t => t.TaskTrackerId == taskTrackerId && t.Status == AHC.CD.Entities.MasterData.Enums.StatusType.Active.ToString(), "TaskTrackerHistories");
        ...
        public async Task<IEnumerable<AHC.CD.Entities.TaskTracker.TaskTracker>> GetAllTasksByUserId(string userAuthId)
        {
            try
            {
                ...
                var tasks = (await taskRepo.GetAllAsync("AssignedTo,TaskTrackerHistories,InsuranceCompanyName")).ToList().OrderBy(c => c.NextFollowUpDate);
        ...
        public async Task<IEnumerable<AHC.CD.Entities.TaskTracker.TaskTracker>> GetAllTasksByProfileId(int profileid)
        {
            try
            {                
                var tasks = (await taskRepo.GetAsync(x => x.ProfileID == profileid, "AssignedTo,TaskTrackerHistories,InsuranceCompanyName")).ToList().OrderBy(c => c.NextFollowUpDate);
                return tasks;
            }
            catch (Exception)
            {
