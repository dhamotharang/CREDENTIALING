    ...
    public class ExpiryNotificationManager : IExpiryNotificationManager
    {
        ...
        public void NotifyExpiries()
        {
            ...
            foreach (var expiry in expiries)
            {
                ...
                foreach (var hospital in expiry.HospitalPrivilegeExpiries.Where(s => s.NotificationDate != null && s.NotificationDate.Value.Date == DateTime.Now.Date))
                {
                    EMailModel emailModel = GetEmailModel(expiry);
                    emailModel.Subject = GetEmailSubject("Hospital Privileges");
                    emailModel.Body = GetEmailBody(GetFullName(expiry), "Hospital Privileges", hospital.AffiliationEndDate.Value, (hospital.AffiliationEndDate.Value - DateTime.Now).TotalDays);
                    EmailSender.SendMail(emailModel);
                }
