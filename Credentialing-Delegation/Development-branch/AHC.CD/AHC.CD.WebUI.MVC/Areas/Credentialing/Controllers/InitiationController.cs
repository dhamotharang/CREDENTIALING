...
namespace AHC.CD.WebUI.MVC.Areas.Credentialing.Controllers
{
    public class InitiationController : Controller
    {
        ...
        [HttpPost]
        public JsonResult InitiateDeCredentialing(int[] InfoidArray, int[] ContractidArray, int[] GrididArray)
        {
            string status = "true";
            var count = 0;
            int id = 0;
            try
            {
                ...
                {
                    id = individualCredentialingManager.InitiateDeCredentialingAsync(Infoid, ContractidArray[count], GrididArray[count], GetUserAuthId());
                    count++;
                }
