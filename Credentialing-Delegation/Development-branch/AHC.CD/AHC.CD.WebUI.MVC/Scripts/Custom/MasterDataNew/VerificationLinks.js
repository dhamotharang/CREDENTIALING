...
MasterDataVerificationLinks.controller('VerificationLinksccontroller', ['$scope', '$http', '$filter', '$rootScope', 'masterDataService', 'messageAlertEngine', function ($scope, $http, $filter, $rootScope, masterDataService, messageAlertEngine) {
    ...
    $scope.saveVerificationLink = function (idx) {
        ...
        for (var i = 0; i < $scope.VerificationLinks.length; i++) {
            ...
            if (addData.Name && $scope.VerificationLinks[i].Name.replace(" ", "").toLowerCase().trim() == addData.Name.replace(" ", "").toLowerCase().trim()) {

                isExist = false;
                $scope.existErr = "This Verification Link already exists";
                break;
            }
        }

        if (!addData.Name) { $scope.error = "Please Enter the Verififcation Link Name"; }
        ...
        if (addData.Name && isExist) {
            $http.post(rootDir + '/MasterDataNew/AddVerificationLink', addData).
                ...
                error(function (data, status, headers, config) {
                    //----------- error message -----------
                    messageAlertEngine.callAlertMessage("verificationlinkerror", "Sorry Unable To Add new Verification Link !!!!", "danger", true);
                    $scope.VerificationLinks.slpice(idx, 1);
                });
        }
