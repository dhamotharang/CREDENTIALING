...
EmailServiceApp.controller("EmailServiceController", function ($rootScope, $timeout, $scope, $http, $filter, messageAlertEngine, ngTableParams, $sce, $filter) {
    ...
    $scope.GetAllGroupMails = function () {
        ...
        $http.get(rootDir + '/EmailService/GetAllGroupMails').
          success(function (data, status, headers, config) {
              try {
                  $scope.GroupMailsForAutosuggest = data;
                  d2.resolve(data);
              } catch (e) {
              }
          }).
    ...
    var promise = $scope.GetAllEmailIds().then(function () {
        $scope.GetAllGroupMails().then(function () {
            //for (i = 0; i <= $scope.GroupMailsForAutosuggest.length;i++) {
            //    $scope.EmailsIds.push($scope.GroupMailsForAutosuggest[i].EmailGroupName);
            //}
        });
    });
    ...
    $http.get(rootDir + '/EmailService/GetAllActiveFollowUpEmails').
       success(function (data, status, headers, config) {
           try {
               ...
               $scope.searchBoxData($scope.followUpEmails);
               $scope.condition = 4;
    ...
    $scope.AddOrSaveEmail = function (Form_Id) {
        ...
        if ($("#" + Form_Id).valid() && checkVariable && !$scope.FileUploadProgress && AttachmentsSize < 3000000) {
