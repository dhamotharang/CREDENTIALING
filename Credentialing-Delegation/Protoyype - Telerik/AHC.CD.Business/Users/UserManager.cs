...
namespace AHC.CD.Business.Users
{
    internal class UserManager : IUserManager
    {
        ...
        public async Task<IEnumerable<CDUser>> GetAllCDUsers()
        {
            try
            {
                var users = await userRepository.GetAsync(u => u.Status != AHC.CD.Entities.MasterData.Enums.StatusType.Inactive.ToString(), "CDRoles, CDRoles.CDRole");
        ...
        public async Task<List<CDUser>> GetAllUsers()
        {
            List<CDUser> cdusers = new List<CDUser>();
            try
            {
                var cduserrolerepo = unitOfWork.GetGenericRepository<CDUserRole>();
                string IncludeProperties = "CDRoles,";
                var res = userRepository.GetAll(IncludeProperties);
                var userroles = cduserrolerepo.GetAll();
                //foreach (var item in res)
                //{
                //    var result = userroles.Select( x => x.CDUserId == item.CDUserID && x.CDRole.Code!="PRO");
                //}
                foreach (var item in userroles)
	            {
                    if(item.CDRoleId != 2 && !cdusers.Any(x=>x.CDUserID==item.CDUserId))
                    {
                        cdusers.Add(res.Where(x => x.CDUserID == item.CDUserId).FirstOrDefault());
                    }
	            }
