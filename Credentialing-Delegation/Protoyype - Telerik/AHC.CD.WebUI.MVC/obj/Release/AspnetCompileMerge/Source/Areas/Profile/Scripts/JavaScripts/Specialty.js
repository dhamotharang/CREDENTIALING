...
profileApp.controller('SpecialtyController', ['$scope', '$rootScope', '$http', 'masterDataService', 'messageAlertEngine', '$filter','profileUpdates', function ($scope, $rootScope, $http, masterDataService, messageAlertEngine, $filter,profileUpdates) {
    ...
    $scope.saveSpecialty = function (specialty) {
        ...
        var TaxonomyCode = $($formData[0]).find($("[name='TaxonomyCode']")).val();
                ...
                success: function (data) {
                    try {
                        if (data.status == "true") {
                            ...
                            data.specialty.Specialty = { ID: data.specialty.SpecialtyID, Name: SpecialtyName, TaxonomyCode: TaxonomyCode };
    ...
    ////Manideep
    //$scope.TaxonomyAutocomplete = function (Taxonomy) {

    //    if (Taxonomy.length == 0) {
    //        $scope.resetTaxonomy();
    //    }

    //    $scope.tempHC.City = Taxonomy;
    //    if (Taxonomy.length > 1 && !angular.isObject(Taxonomy)) {
    //        locationService.getLocations(Taxonomy).then(function (val) {
    //            $scope.Taxonomy = val;
    //        });
    //    } else if (angular.isObject(Taxonomy)) {
    //        $scope.setTaxonomy(Taxonomy);
    //    }
    //};
    //function showTaxonomy(ele) {
    //    $(ele).parent().find(".TaxonomySelectAutoList").first().show();
    //}
    //$scope.selectedLocation = function (Taxonomy) {
    //    $scope.setTaxonomy(Taxonomy);
    //    $(".TaxonomySelectAutoList").hide();
    //};
    //$scope.setTaxonomy = function (Taxonomy) {
