profileApp.controller('practiceLocationController', ['$scope', '$rootScope', '$http', '$filter', 'masterDataService', 'locationService', 'messageAlertEngine', 'profileUpdates', function ($scope, $rootScope, $http, $filter, masterDataService, locationService, messageAlertEngine, profileUpdates) {
    ...
    $scope.editGeneralInfo = function (PracticeLocationDetail) {
        ...
        if ($scope.tempSecondObject.Facility.FacilityName == null) {
            FacilityName = "";
        } else {
            FacilityName = $scope.tempSecondObject.Facility.FacilityName;
            $scope.tempSecondObject.FacilityID = $scope.tempSecondObject.Facility.FacilityID;
        }
        if ($scope.tempSecondObject.Facility.Street == null) {
            Street = "";
        } else {
            Street = $scope.tempSecondObject.Facility.Street;
        }
        if ($scope.tempSecondObject.Facility.Building == null) {
            Building = "";
        } else {
            Building = $scope.tempSecondObject.Facility.Building;
        }
        if ($scope.tempSecondObject.Facility.City == null) {

            City = "";

        } else {

            City = $scope.tempSecondObject.Facility.City;

        }

        if ($scope.tempSecondObject.Facility.State == null) {

            State = "";

        } else {

            State = $scope.tempSecondObject.Facility.State;
    ...
    $scope.dayOffToggel = function (PracticeDay) {
        ...
        if (PracticeDay.DayOff == 'NO' && PracticeDay.DailyHours[0].StartTime != 'Invalid Date') {
            return;
        }
        else if ((PracticeDay.DayOff == 'NO' && PracticeDay.DailyHours[0].StartTime == 'Invalid Date') || (PracticeDay.DayOff == 'NO' && PracticeDay.DailyHours[0].StartTime == 'Day Off')) {
    ...
    $scope.validateMidLevelProviderPractitioner = function (dataObject, dataScope, action) {
        ...
        //else if (dataObject.NPINumber != null && dataObject.NPINumber.length != 10) {
        //    status = false;
