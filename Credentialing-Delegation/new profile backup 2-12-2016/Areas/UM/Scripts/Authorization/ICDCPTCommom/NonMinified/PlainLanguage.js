$(function () {
    ...
    $(".edit-selected-plain-languages").off("click", ".editPlainLangBtn").on("click", ".editPlainLangBtn", function () {
        ...
        var editselectedli = '<textarea class="form-control plangtextarea" rows="5">' + $Description + '</textarea>' +
                             '<input type="submit" value="SAVE" style="margin-top: 2px;" class="btn btn-success btn-xs pull-right savePlainLangBtn">';
        $row.children('.plainLang').html(editselectedli);
        $('.list-of-plain-languages').show();
        $('#plainLanguageFilter').val("");
    });
    /*---------------------------------------------------------------------------------------------*/
    ...
    $("#plainlangContainer").off('keyup', '#plainLanguageFilter').on('keyup', '#plainLanguageFilter', function () {
        var optn = '';
        var li = '';
        $(this).attr('autocomplete', 'off');
        if ($(this).val()) {
            var result = GetSearchResult($(this).val(), MasterPlainLanguages);
            if (result) {
                for (i = 0; i < result.length - 1; i++) {
                    optn = optn + '<option value="' + result[i].PlainLanguageName + '">' + result[i].PlainLanguageName + '</option>';
                    li = li + '<li class="list-group-item"><a>' + result[i].PlainLanguageName + '</a></li>';
                ...
                if (li) {
                    ...
                    $("#plainlanglist").append("<li>NO DATA AVAILABLE</li>");
                }
            }
        }
        else
    ...
    function GetSearchResult(key,obj) {
        try{
            if(obj && key)
            {
                var val = [];
                ...
                {
                    if (ind=obj[i].PlainLanguageName.toLowerCase().search(key.toLowerCase())===0)
                    {
                        str=obj[i].PlainLanguageName[ind]
                        exact.push(obj[i]);
                    }
                    else if (obj[i].PlainLanguageName.toLowerCase().search(key.toLowerCase()) !== -1)
                    {
                        partial.push(obj[i]);
                    }
                }
                return val = exact.concat(partial);
            }
        }
        catch(e){
            return null;
