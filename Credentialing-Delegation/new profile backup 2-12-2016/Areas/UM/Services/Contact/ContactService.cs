...
namespace PortalTemplate.Areas.UM.Services.Contact
{
    public class ContactService : IContactServices
    {
        ...
        public PortalTemplate.Areas.Portal.Models.Contact.AuthorizationContactViewModel ViewContactServices(int ContactID)
        {
            PortalTemplate.Areas.Portal.Models.Contact.AuthorizationContactViewModel authorizationContactViewModel = new PortalTemplate.Areas.Portal.Models.Contact.AuthorizationContactViewModel();
            Task<string> authorizationContact = Task.Run(async () =>
            {
                string msg = await serviceUtility.GetDataFromService(baseURL, "api/Contact/GetAuthorizationContactByAuthorizationContactID?ContactID=" + ContactID);
                return msg;
            });
            if (authorizationContact.Result != null)
            {
                authorizationContactViewModel = JsonConvert.DeserializeObject<PortalTemplate.Areas.Portal.Models.Contact.AuthorizationContactViewModel>(authorizationContact.Result);
            }
            return authorizationContactViewModel;
        }
