(function () {
    ...
    chromeTabs = {
            ...
            return $tabs.sortable({
                ...
                start: function (e, ui) {
                    ...
                    if (!$(ui.item).hasClass('chrome-tab-current')) {
                        ...
                        return $tabs.sortable('option', 'zIndex', $tabs.length + 0);
                    }
                },
                stop: function (e, ui) {
        ...
        fixTabSizes: function ($shell) {
            ...
            width = Math.max($shell.data().minWidth,
            Math.min($shell.data().maxWidth, width));
            $tabs.css({
                width: width
            });
        ...
        closeTab: function ($shell, $tab) {
            if (innerTabs.length > 2) {
                if ($tab.hasClass('chrome-tab-current')) {
                    if ($tab.prev().length) {
                        chromeTabs.setCurrentTab($shell, $tab.prev());
                        setBreadCrum($tab.prev()[0].childNodes[3].innerHTML.toLowerCase());
                    } else if ($tab.next().length) {
                        chromeTabs.setCurrentTab($shell, $tab.next());
                    }
                }
                $tab.remove();
                var index = innerTabs.indexOf($tab.attr('data-id'));
                innerTabs.splice(index, 1);
                $("#" + $tab.attr('data-id')).removeAttr("style");
            }
        ...
        updateTab: function ($shell, $tab, tabData) {
            ...
            $tab.attr("data-id", tabData.data.id);
