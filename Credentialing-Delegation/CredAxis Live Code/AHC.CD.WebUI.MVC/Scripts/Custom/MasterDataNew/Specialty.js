...
masterDataSpecialtys.controller('masterDataSpecialtysController', ['$scope', '$http', '$filter', '$rootScope', 'masterDataService', 'messageAlertEngine', function ($scope, $http, $filter, $rootScope, masterDataService, messageAlertEngine) {
    ...
    $scope.editSpecialtys = function (Specialty) {        
        //$scope.tempSpecialtys.TaxonomyCode = "";
    ...
    $scope.saveSpecialtys = function (idx) {
        ...
        var addData = {
            ...
            TaxonomyCode: $scope.tempSpecialtys.TaxonomyCode,
            StatusType: 1,
        };
        ...
        for (var i = 0; i < $scope.Specialtys.length; i++) {
            console.log(i, $scope.Specialtys[i].Name);
            $scope.Specialtys[i].Name == null ? "" : $scope.Specialtys[i].Name;
            
            if ($scope.Specialtys[i].TaxonomyCode == null) {
                $scope.Specialtys[i].TaxonomyCode = "";
            }
            ...
            //$scope.Specialtys[i].TaxonomyCode == null ? "" : $scope.Specialtys[i].TaxonomyCode;
            if (addData.TaxonomyCode && $scope.Specialtys[i].TaxonomyCode.replace(" ", "").toLowerCase() == addData.TaxonomyCode.replace(" ", "").toLowerCase()) {

                isExist = false;
                $scope.existError = "The Taxonomy Code is Exist";
                break;
            }
        }
        if (!addData.Name) { $scope.error = "Please Enter the Name"; }
        //if (!addData.TaxonomyCode) { $scope.error1 = "Please Enter the TaxonomyCode"; }
    ...
    $scope.updateSpecialtys = function (idx) {
        ...
        for (var i = 0; i < $scope.Specialtys.length; i++) {
            ...
            if (updateData.TaxonomyCode != null && $scope.Specialtys[i].TaxonomyCode != null) {
                if (updateData.TaxonomyCode && $scope.Specialtys[i].SpecialtyID != updateData.SpecialtyID && $scope.Specialtys[i].TaxonomyCode.replace(" ", "").toLowerCase() == updateData.TaxonomyCode.replace(" ", "").toLowerCase()) {
            ...
            if ($scope.Specialtys[i].SpecialtyID == updateData.SpecialtyID) {
                idx = i;
            }
        }
        if (!updateData.Name) { $scope.error = "Please Enter the Name"; }
        //if (!updateData.TaxonomyCode) { $scope.error1 = "Please Enter the TaxonomyCode"; }
