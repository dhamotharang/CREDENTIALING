    ...
    public class DocumentRepositoryController : Controller
    {
        ...
        public async Task<ActionResult> AddOtherDocumentAsync(int profileId, OtherDocumentViewModel otherDocument)
        {
            ...
            try
            {
                if (ModelState.IsValid && otherDocument.File != null)
                {
                    ...
                    ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Document Repository - Other Documents", "Added");
                    ...
                    else
                    {
                        ...
                        if (dataModelOtherDocument.IsPrivate == false)
                        {
                            if (document == null)
                            {
                                ...
                                document.OldFilePath = dataModelOtherDocument.DocumentPath;
                            }
                            ...
                            string documentpath = profileManager.UpdateOtherDocumentAsync(profileId, dataModelOtherDocument, document);
                            otherDocument.DocumentPath = documentpath;
                        }
                    }
                }
                else
        ...
        public async Task<ActionResult> RemoveOtherDocument(int profileId, int OtherDocumentID)
        {
            ...
            try
            {
                ...
                await profileManager.RemoveOtherDocumentAsync(profileId, dataModelOtherDocument,UserAuthID);
                ChangeNotificationDetail notification = new ChangeNotificationDetail(profileId, User.Identity.Name, "Document Repository-Other Document Details", "Removed");
                await notificationManager.SaveNotificationDetailAsyncForAdd(notification, isCCO);
            }
            catch (DatabaseValidationException ex)
            {
