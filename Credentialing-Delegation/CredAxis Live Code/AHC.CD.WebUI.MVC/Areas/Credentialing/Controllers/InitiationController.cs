...
namespace AHC.CD.WebUI.MVC.Areas.Credentialing.Controllers
{
    public class InitiationController : Controller
    {
        ...
        #region New
        //[HttpPost]
        //public JsonResult DashboardInitiateReCredentialing(int id, CredentialingInitiationInfoViewModel credentialingInitiationInfo, int CredentialingContractRequestsArray, int LOBID, int ContractGridID)
        //{
        //    string status = "true";
        ...
        //    try
        //    {
        //        id1 = individualCredentialingManager.DashBoardInitiateReCredentialingAsync(id, dataCredentialingInitiationInfo, GetUserAuthId(), CredentialingContractRequestsArray, LOBID, ContractGridID);
        //    }
        //    catch (DatabaseValidationException ex)
        //    {
        //        errorLogger.LogError(ex);
        ...
        [HttpPost]
        public JsonResult InitiateDeCredentialing(int[] InfoidArray, int[] ContractidArray, int[] GrididArray, int reasonId, String decredDoc)
        {
            string status = "true";
            var count = 0;
            int id = 0;
            ...
            try
            {
                //DocumentDTO documentDTO = CreateDocument(decredDoc.DecredentialingDocument);
                //DocumentDTO documentDTO = new DocumentDTO();
                //documentDTO.DocRootPath = decredDoc;
                //documentDTO.FileName = UniqueKeyGenerator.GetUniqueKey() + Document.FileName;
                //documentDTO.DocRootPath = HttpContext.Request.MapPath("~/ApplicationDocuments/DecredentialingReasons/");
                
                //individualCredentialingManager.SaveOrUpdateProviderDocument(documentDTO);
                //message = CompletionMessages.CV_UPLOADED_SUCCESSFULLY;
                
                foreach (int Infoid in InfoidArray)
                {
                    id = individualCredentialingManager.InitiateDeCredentialingAsync(Infoid, ContractidArray[count], GrididArray[count], GetUserAuthId(), reasonId, decredDoc);
                    count++;
                }
        ...
        #region CredentialingContractGrid


        public async Task<string> getCredentialingContractGridForDecred(int ProviderID, int PlanID)
        {
            IEnumerable<Object> data = null;
            try
            {
                data = await individualCredentialingManager.getAllCredentialinginfoByContractGrid(ProviderID, PlanID);
            }
            catch (Exception ex)
            {
                throw ex;
            }
            return JsonConvert.SerializeObject(data, new JsonSerializerSettings { DateTimeZoneHandling = DateTimeZoneHandling.Local });
        }
        ...
        public async Task<string> getCredentialingContractRequestForAllPlan(int ProviderID, int[] PlanIDs)
        {
            IEnumerable<Object> data = null; ;
            try
            {
        ...
        public async Task<ActionResult> DecredentialingSummary(int id)
        {
            ...
            foreach (CredentialingInfo c in data)
            {
                ...
                {
                    ViewBag.infoSummary = JsonConvert.SerializeObject(c, new JsonSerializerSettings
                        {
