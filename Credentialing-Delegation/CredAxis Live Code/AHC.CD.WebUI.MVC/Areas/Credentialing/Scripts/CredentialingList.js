...
credentialingList.controller('credentialingListController', ['$scope', '$http', '$q', '$filter', 'ngTableParams', '$rootScope', 'messageAlertEngine', '$timeout', function ($scope, $http, $q, $filter, ngTableParams, $rootScope, messageAlertEngine, $timeout) {
    $scope.data = [];
        ...
        if ($scope.data.length != 0) {
            ...
            if (data.CredentialingActivityLogs != null && data.CredentialingActivityLogs.length != 0) {
                if (data.Credentialing == "Credentialing" || data.Credentialing == "ReCredentialing") {
                    ...
                    //for (var i = 0; i < data.CredentialingActivityLogs.length; i++) {
                    //    switch (data.CredentialingActivityLogs[i].Activity) {
                    //        case "Initiation":
                    //            if (data.CredentialingActivityLogs[i].ActivityStatus == "Completed") {
    ...
    $scope.clearActionfortab = function () {
        $scope.SearchProviderPanelToggleDown('SearchProviderPanel');
        $scope.data = "";
        $scope.showInit = false;
    }
    ...
    function DecredentialingReasons() {
        var defer = $q.defer();
    ...
    $scope.getPlanReport = function (obj) {
        $q.when(DecredentialingReasons()).then(function (response) {
            //$scope.DocPath = obj.DecredentialingReasonDocumentPath != null ? obj.DecredentialingReasonDocumentPath : '';
            //edited by pritam
            ...
            for (var i = 0; i < obj.CredentialingContractRequests.length; i++) {
                for (var j = 0; j < obj.CredentialingContractRequests[i].ContractGrid.length; j++) {
                    if (obj.CredentialingContractRequests[i].ContractGrid[j].ContractGridStatus == 'Inactive') {
                        $scope.planReportList.push(obj.CredentialingContractRequests[i].ContractGrid[j]);
                        //if (typeof $scope.reason !== "undefined" || $scope.reason != null){                       
                        //    $scope.planReportList[0].DecredReason= $scope.reason;
                        //}
                        //else {
                        //    $scope.planReportList[0].DecredReason = "";
                        //}
                    }
                }
            }
            for (var j = 0; j < $scope.planReportList.length; j++) {
                for (var i = 0; i < $scope.reasonsForDeCred.length; i++) {
                    if ($scope.planReportList[j].DecredentialingReasonId == $scope.reasonsForDeCred[i].DecredentialingReasonId) {
                        $scope.planReportList[j].DecredentialingReason = $scope.reasonsForDeCred[i].Reason;
                    }
                }
            }
    ...
    $('#bell').click(function () {
        //$("#alertArea").toggle("slow", function () {
        //});
        var isVisible = $("#alertArea").is(":visible");
        var isHidden = $("#alertArea").is(":hidden");

        if (isVisible) {
            $("#alertArea").slideUp();
        }
        if (isHidden) {
            $("#alertArea").hide().slideDown('slow');
        }
    });
