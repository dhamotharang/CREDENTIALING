...
define(['Util/MasterProfileApp'], function (app) {
    'use strict';
    app.register.controller('SpecialtyController', ['$scope', '$rootScope', '$http', 'httpq', 'masterDataService', 'messageAlertEngine', '$filter', 'profileUpdates',
      function ($scope, $rootScope, $http, httpq, masterDataService, messageAlertEngine, $filter, profileUpdates) {
          ...
          $(function () {
              ...
              if (!$rootScope.MasterData.SpecialtyBoards) {
                  httpq.get(rootDir + "/Profile/MasterData/GetAllSpecialtyBoards").then(function (data) {
                      $rootScope.MasterData.SpecialtyBoards = data;
                          ...
                          try {
                              if (data.status == "true") {
                                  if (data.specialty.PreferenceType == 1) {
                                      $scope.setPrimary();
                                      for (var i = 0; i < $rootScope.MasterProfile.Specialty.SpecialtyDetails.length; i++) {
                                          if ($rootScope.MasterProfile.Specialty.SpecialtyDetails[i].SpecialtyPreference == "Primary") {
                                              $rootScope.MasterProfile.Specialty.SpecialtyDetails[i].SpecialtyPreference = "Secondary";
                                              $rootScope.MasterProfile.Specialty.SpecialtyDetails[i].PercentageOfTime = 50;
                                          }
                                      }
                                  }
                                  //--------- speciality object binding ------------
                                  for (var i in $rootScope.MasterData.Specialties) {
                                      if ($rootScope.MasterData.Specialties[i].SpecialtyID == data.specialty.SpecialtyID) {
                                          data.specialty.Specialty = angular.copy($rootScope.MasterData.Specialties[i]);
                                          break;
                                      }
                                  }
                                  ...
                                  $rootScope.MasterProfile.Specialty.SpecialtyDetails.push(data.specialty);
                                  myData = data;
          ...
          $scope.setErrorBit = function (name) {
              ...
              if (name == "") {
                  ...
                  for (var i = 0; i < $rootScope.MasterData.Specialties.length; i++) {
                      if ($rootScope.MasterData.Specialties[i].Name == name) {
                          $scope.tempObject.Specialty.TaxonomyCode = angular.copy($rootScope.MasterData.Specialties[i].TaxonomyCode);

                      }
                  }
                  console.log($scope.tempObject.Specialty.TaxonomyCode);
                  //if (Length == $rootScope.MasterData.Specialties[i].length)
                  //alert($)
                  $scope.errormessage = false;
                  $scope.hasError = false;
              }
                          ...
                          try {
                              if (data.status == "true") {
                                  if (data.specialty.PreferenceType == 1) {
                                      $scope.setPrimary();
                                      for (var i = 0; i < $rootScope.MasterProfile.Specialty.SpecialtyDetails.length; i++) {
                                          if ($rootScope.MasterProfile.Specialty.SpecialtyDetails[i].SpecialtyPreference == "Primary" && i != IndexValue) {
                                              $rootScope.MasterProfile.Specialty.SpecialtyDetails[i].SpecialtyPreference = "Secondary";
                                              $rootScope.MasterProfile.Specialty.SpecialtyDetails[i].PercentageOfTime = 50;
                                          }
                                      }
                                  ...
                                  $rootScope.MasterProfile.Specialty.SpecialtyDetails[IndexValue] = data.specialty;
                                  $scope.ErrorInSpecialtyDetails = false;
                                  if ($scope.visibilityControl == (IndexValue + '_editboardSpecialty')) {
          ...
          $scope.setPrimary = function () {
              try {
                  for (var i = 0; i < $rootScope.MasterProfile.Specialty.SpecialtyDetails.length ; i++) {
                      $rootScope.MasterProfile.Specialty.SpecialtyDetails[i].PreferenceType = "2";
                  }
              }
              catch (e) { }
          };
              ...
              if (validationStatus) {
                  ...
                  $.ajax({
                      ...
                      success: function (data) {
                          try {
                              if (data.status == "true") {
                                  $rootScope.MasterProfile.Specialty.PracticeInterest = data.practiceInterest;
