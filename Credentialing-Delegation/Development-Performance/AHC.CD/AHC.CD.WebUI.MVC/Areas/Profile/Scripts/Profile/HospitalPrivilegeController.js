...
define(['Util/MasterProfileApp'], function (app) {
    'use strict';
    app.register.controller('HospitalPrivilegeController', ['$scope', '$rootScope', '$http', 'httpq', 'messageAlertEngine', '$filter', 'profileUpdates',
      function ($scope, $rootScope, $http, httpq, messageAlertEngine, $filter, profileUpdates) {

          //-------------------------- MAster Data Get Function --------------------------------
          $(function () {
              if (!$rootScope.MasterData.StaffCategories) {
                  httpq.get(rootDir + "/Profile/MasterData/GetAllStaffCategories").then(function (data) {
                      $rootScope.MasterData.StaffCategories = data;
                      //$rootScope.MasterData.StaffCategories = data;
                  });
              }
              if (!$rootScope.MasterData.AdmittingPrivileges) {
                  httpq.get(rootDir + "/Profile/MasterData/GetAllAdmittingPrivileges").then(function (data) {
                      $rootScope.MasterData.AdmittingPrivileges = data;
                      //$rootScope.MasterData.AdmittingPrivileges = data;
                  });
              }
          ...
          $(function () {
              if ($rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails) {
                  $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails = _.filter($rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails, function (o) { return o.Status != 'Inactive'; });
              }
              _.forEach($rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails, function (n) {
          ...
          $scope.getHospitalNameForProvider = function (findData) {
              try {
                  return $rootScope.MasterData.Hospitals.filter(function (hospitals) { return hospitals.HospitalID == findData })[0];
              }
              catch (e) { }
          }
          ...
          $scope.getStaffCategoryForProvider = function (findData) {
              try {
                  return $rootScope.MasterData.StaffCategories.filter(function (staffCategories) { return staffCategories.StaffCategoryID == findData })[0];
              }
              catch (e) { }
          }
          ...
          $scope.getHospitalLocationForProvider = function (valLocaton, findData) {
              try {
                  $scope.locations = $rootScope.MasterData.Hospitals.filter(function (hospitals) { return hospitals.HospitalID == valLocaton })[0].HospitalContactInfoes;
          ...
          $scope.getAdmittingPrivilageStatusForProvider = function (findData) {
              try {
                  return $rootScope.MasterData.AdmittingPrivileges.filter(function (admittingPrivileges) { return admittingPrivileges.AdmittingPrivilegeID == findData })[0];
              }
              catch (e) { }
          }
          ...
          $scope.setPrimary = function () {
              try {
                  for (var i = 0; i < $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails.length ; i++) {
                      $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails[i].PreferenceType = "2";
                  }
              }
              catch (e) { }
          };
          ...
          $scope.saveHopitalPrivilegeInformation = function (hospitalPrivilegeInformation) {
              ...
              if (validationStatus) {
                  if (hospitalPrivilegeInformation.HospitalPrivilegeYesNoOption == '1') {
                      ...
                      try {
                          $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails = [];
                      } catch (e) { }
                  }
                  $.ajax({
                      url: url,
                      ...
                      success: function (data) {
                          ...
                          try {
                              if (data.status == "true") {
                                  try {
                                      if ($rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation != null) {
                                          $scope.HospitalInformationPendingRequest = true;
                                      }
                                      $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation
                                      $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation = angular.copy(data.hospitalPrivilegeInformation);
                                  }
                                  catch (e)
                                  { }
                  ...
                  $.ajax({
                      ...
                      success: function (data) {
                          ...
                          try {
                              ...
                              if (data.status == "true") {
                                  if ($scope.visibilityControl == 'addhospitalPrivilegeInformation') {
                                      for (var i = 0; i < $rootScope.MasterData.AdmittingPrivileges.length; i++) {

                                          if ($rootScope.MasterData.AdmittingPrivileges[i].AdmittingPrivilegeID == data.hospitalPrivilegeDetail.AdmittingPrivilegeID) {
                                              hospitalPrivilegeInformation.AdmittingPrivilege = angular.copy($rootScope.MasterData.AdmittingPrivileges[i]);
                                          }
                                      }
                                      for (var i = 0; i < $rootScope.MasterData.StaffCategories.length; i++) {

                                          if ($rootScope.MasterData.StaffCategories[i].StaffCategoryID == data.hospitalPrivilegeDetail.StaffCategoryID) {
                                              hospitalPrivilegeInformation.StaffCategory = angular.copy($rootScope.MasterData.StaffCategories[i]);
                                          }
                                      }
                                      ...
                                      $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails.push(hospitalPrivilegeInformation);
                                      $scope.operateCancelControl('');
                                      ...
                                      //var obj = $filter('filter')($rootScope.MasterData.AdmittingPrivileges, { AdmittingPrivilegeID: data.hospitalPrivilegeDetail.AdmittingPrivilegeID })[0];

                                      hospitalPrivilegeInformation.Hospital.HospitalName = $scope.getHospitalNameForProvider(hospitalPrivilegeInformation.Hospital.HospitalID).HospitalName;
                                      hospitalPrivilegeInformation.HospitalContactInfo.LocationName = $scope.getHospitalLocationForProvider(hospitalPrivilegeInformation.Hospital.HospitalID, hospitalPrivilegeInformation.HospitalContactInfo.HospitalContactInfoID).LocationName;
                                      hospitalPrivilegeInformation.HospitalPrevilegeLetterPath = data.hospitalPrivilegeDetail.HospitalPrevilegeLetterPath;
                                      $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails[IndexValue] = hospitalPrivilegeInformation;
                                      ...
                                      for (var i = 0; i < $rootScope.MasterData.AdmittingPrivileges.length; i++) {
                                          ...
                                          if ($rootScope.MasterData.AdmittingPrivileges[i].AdmittingPrivilegeID == data.hospitalPrivilege.AdmittingPrivilegeID) {
                                              hospitalPrivilegeInformation.AdmittingPrivilege.Title = angular.copy($rootScope.MasterData.AdmittingPrivileges[i].Title);
                                          }
                                      }
                                      ...
                                      for (var i = 0; i < $rootScope.MasterData.StaffCategories.length; i++) {
                                          ...
                                          if ($rootScope.MasterData.StaffCategories[i].StaffCategoryID == data.StaffCategoryID) {
                                              hospitalPrivilegeInformation.StaffCategory.Title = angular.copy($rootScope.MasterData.StaffCategories[i].Title);
                                          }
                                      }
                  ...
                  $.ajax({
                      ...
                      success: function (data) {
                          try {
                              ...
                              if (data.status == "true") {
                                  var obj = $filter('filter')($rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails, { HospitalPrivilegeDetailID: data.hospitalPrivilege.HospitalPrivilegeDetailID })[0];
                                  myData = data;
                                  $rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails.splice($rootScope.MasterProfile.HospitalPrivilege.HospitalPrivilegeInformation.HospitalPrivilegeDetails.indexOf(obj), 1);
